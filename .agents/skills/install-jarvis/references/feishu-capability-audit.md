...
安装授权统一由 [feishu-authorization.md](feishu-authorization.md) 和 `scripts/jarvis-lark-auth` 负责。此处只读验证“lark-cli 当前默认身份实际能做什么”：
...
安装交付需要验证内置功能所需权限和以下读取能力。审计内不申请权限：源码安装发现 OAuth 缺口时返回安装授权步骤统一补齐；桌面后台任务记录缺口并指向安装页的重新授权入口，不自行发起另一条授权流程。应用侧或单份资源无权需分别处理。保留原始错误并如实记录未完成项。源码安装有清单时将 `install.feishu-capabilities` 及相应世界模型项保持未勾选；桌面任务记录在工作稿与任务结果，不阻止进入已经就绪的应用，也不把建模描述为完成。

| 能力 | 推荐只读探针 | 通过标准 |
|---|---|---|
| 内置功能 OAuth 权限 | `./scripts/jarvis-lark-auth check` | 退出码 0；已有登录也必须检查，不能以 token 有效代替 |
| 当前用户与 Bot 身份 | `auth status --json --verify` | user、bot 均验证成功，user open_id 与 principal 一致 |
...
| 日历与待办 | 按 `lark-calendar` / `lark-task` 查询本人近期日程与未完成待办 | 调用成功；空结果和缺权限分开 |
| 视频会议、智能纪要与妙记 | 按 `lark-meeting` 查询近期会议、搜索本人可见妙记；有候选时读取智能纪要、妙记总结和逐字稿 | 接口成功；没有候选记未实测，单篇访问失败不能归为 OAuth 缺失 |
| 内嵌资源 | 根据实际候选按对应 Skill 读取电子表格、多维表格、知识库、幻灯片或画板 | 有候选则实测；无候选记覆盖不足，不能宣称已验证 |
| CC Connect Bot 读取 | 以 bot 身份运行群列表；有候选群时再运行群成员和群消息轻量探针 | API 调用成功；零群表示 Bot 尚无可见群，不伪装成权限失败，留给端到端验收覆盖 |

命令参数、时间窗口和候选选择由 Agent 根据机器现状调整；所有读取使用 lark-cli 当前默认身份，需要用户或 Bot 身份时显式带正确的 `--as user|bot`。
