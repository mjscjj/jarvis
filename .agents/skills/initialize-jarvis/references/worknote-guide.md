...
2. Principal 身份、职责、直属上级和 Git author。
...
按 `Principal → Project → Person → KeyMatter/ManagedResource → Group → RelationFact → baseline Fact` 的依赖顺序逐项处理。每次写入后立即使用对应 get/list/query 命令读回，并把真实 ID 和结果写到工作稿及 `CHECKLIST.md`。
