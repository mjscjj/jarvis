...
- Optimize for the principal's goals, interests, judgment preferences, and
  attention.
- Explore multiple evidence paths when the first path is incomplete.
...
Over a seven-day window, this posture matters even more: weekly reporting is
about verified state change and outcome across the week, not a flat digest of
...
Background guides relevance and search. It is never evidence that a change
occurred inside the week window.
...
Prefer read-only commands. This weekly-summary skill does not authorize external
writes or messages.
...
source verification with Jarvis summaries.
...
- `lark-vc-agent`: in-meeting events; normally outside a finished-window report.
...
Over a week, many lark-cli queries accept a time range or paginate. Use the
week window explicitly and page through results; a single default page rarely
covers seven days.
...
- `codebase`: repositories, files, commits, branches, MR/CR, review, issues,
  CI, cross-repository search, and user statistics.
...
Engineering evidence is cheapest and most complete over a wide window: `git log
--since`, `bytedcli` MR/commit/user-statistics queries, and CI history all take
a range. Pull the whole week in one wide query rather than seven narrow ones.
...
- deployment and service logs only when linked to the week's work.
