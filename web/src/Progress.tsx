...
import { Alert, Button, Card, DatePicker, Empty, Segmented, Space, Spin, Table, Tabs, Tag, Typography } from 'antd'
  ...
  const generate = async (scope: DailyDigestScope, scopeId: string) => {
    ...
    try {
      ...
      setDailyItems((current) => {
        ...
        if (existing) {
          return current.map((item) => item === existing ? { ...item, status: 'generating', error_detail: null } : item)
        }
        return [...current, {
          id: 0, scope, scope_id: scopeId, digest_date: selectedDate, summary: '', status: 'generating',
          source_count: 0, engine: scope === 'person' ? 'codex' : 'qwen', error_detail: null,
  ...
  const digestCard = (title: string, scope: DailyDigestScope, scopeId: string) => {
    ...
    return (
      <Card
        ...
        extra={<Button size="small" loading={isGenerating} onClick={() => generate(scope, scopeId)}>{item ? '重算' : '生成'}</Button>}
      >
        {status === 'failed' ? (
          ...
          <>
            ...
            <Text type="secondary">
              ...
              {` · ${item.source_count} 条来源 · ${item.engine}`}
            </Text>
          </>
        ) : isGenerating ? (
            ...
            children: (
              <>
                ...
                <Spin spinning={dailyLoading}>
                  <Space direction="vertical" size={16} style={{ width: '100%', marginTop: 12 }}>
                    {profile && digestCard('我的进度', 'person', profile.open_id)}
