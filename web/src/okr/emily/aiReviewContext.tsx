...
export function PreviewReviewButton({ target, label, className = '' }: { target: PreviewReviewTarget; label: string; className?: string }) {
  ...
  return <span className="inline-flex flex-col items-end">
    <button type="button" disabled={!ready || running} onClick={() => void start()} className={`rounded-md border border-sky-200 bg-sky-50 px-1.5 py-0.5 text-[10px] font-semibold text-sky-700 hover:bg-sky-100 disabled:cursor-not-allowed disabled:opacity-45 ${className}`}>
