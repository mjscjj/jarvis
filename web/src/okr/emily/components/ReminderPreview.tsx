...
export function ReminderPreview({ quarter, week, onClose }: { quarter: string; week: string; onClose: () => void }) {
  ...
  return (
    <section className="mb-3 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm" aria-label="周一催办预览">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 px-4 py-3">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-xs font-semibold text-slate-800">周一催办预览</h2>
        ...
        <div className="flex items-center gap-2">
          <button type="button" disabled={generating} onClick={() => void createBatch()} className="rounded-md border border-blue-200 bg-blue-50 px-2.5 py-1 text-[11px] font-medium text-blue-700 hover:bg-blue-100 disabled:cursor-wait disabled:opacity-50">{generating ? '生成中…' : '生成审核批次'}</button>
      ...
      {visibleState.kind === 'ready' && (
        <div className="p-4">
          ...
          <div className="mb-3 flex flex-wrap gap-x-5 gap-y-1 text-[11px] text-slate-500">
            <span><b className="mr-1 text-base font-semibold text-slate-800">{visibleState.data.summary.needsReminderOwnerCount}</b>人待提醒</span>
          ...
          <div className="mt-4 border-t border-slate-100 pt-3">
            ...
            {batches.length === 0 ? (
              <p className="rounded-md bg-slate-50 px-3 py-3 text-center text-[11px] text-slate-400">尚无批次，周一定时任务或手动生成后会出现在这里。</p>
            ) : (
              <div className="space-y-1.5">
                {batches.slice(0, 6).map((batch) => (
                  <div key={batch.id} className="flex flex-wrap items-center gap-x-3 gap-y-1 rounded-md border border-slate-100 px-3 py-2 text-[10px] text-slate-500">
                    ...
                    <span className="font-medium text-slate-600">{batch.trigger === 'scheduler' ? '周一定时' : '手动生成'}</span>
