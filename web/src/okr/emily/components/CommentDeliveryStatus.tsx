...
export function CommentDeliveryStatus({ comment }: { comment: PageComment }) {
 ...
 return <div className="mt-1 text-[10px] text-slate-500">
  {items.map((item) => <div key={item.email}>
   {item.name}：{item.status === 'delivered' ? '已提醒' : item.status === 'unknown' || item.status === 'sending' ? '发送结果待核验' : '提醒未完成'}
   {item.error && <span className="ml-1 text-amber-700">{item.error}</span>}
   {['pending', 'failed'].includes(item.status) && <button type="button" disabled={busy} onClick={() => void retry(item.email)} className="ml-2 text-indigo-600 disabled:opacity-50">重试提醒</button>}
