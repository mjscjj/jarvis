...
import { getTask, getWebConfig } from '../../api'
import {
  ...
  startRegionalAutoMatch,
  updateRegionalDemand,
} from './api'
import { CommentableField, CommentableInput, CommentableText, CommentableTextarea, CommentInteractionProvider, commentTargetElementId, scrollToCommentSource, type PendingCommentSelection } from './commenting'
...
import {
  buildRegionalCommentDocumentOrder,
...
import { projectRegionalAlignment } from './regionalAlignmentProjection'
import type {
  CommentTarget,
  Kr,
  KrOwner,
...
function CollapsibleBlock({ title, subtitle, level, commentTarget, instanceKey, action, children }: { title: string; subtitle?: string; level: 'primary' | 'secondary'; commentTarget?: CommentTarget; instanceKey?: string; action?: React.ReactNode; children: React.ReactNode }) {
  ...
  return <div>
    <div className={`flex flex-wrap items-start gap-3 ${primary ? 'mb-5 border-b border-slate-200 pb-4' : 'mb-4'}`}>
      <div className="flex min-w-0 flex-1 items-start gap-2 text-left">
        <button type="button" aria-label={`${open ? '收起' : '展开'} ${title}`} aria-expanded={open} onClick={() => setOpen((value) => !value)} className="mt-0.5 shrink-0 text-indigo-400"><span className={`block transition-transform ${open ? 'rotate-90' : ''}`}>›</span></button>
        {commentTarget && instanceKey ? <CommentableText target={commentTarget} text={`${title}${subtitle ? `\n${subtitle}` : ''}`} instanceKey={instanceKey}><span><span className={`block font-semibold text-slate-900 ${primary ? 'text-lg' : 'text-base'}`}>{title}</span>{subtitle && <><span aria-hidden>{'\n'}</span><span className="mt-1 block text-xs font-normal text-slate-400">{subtitle}</span></>}</span></CommentableText> : <span><span className={`block font-semibold text-slate-900 ${primary ? 'text-lg' : 'text-base'}`}>{title}</span>{subtitle && <span className="mt-1 block text-xs font-normal text-slate-400">{subtitle}</span>}</span>}
      </div>
...
function alignmentTarget(id: string, title: string): CommentTarget {
  return { type: 'alignment_item', id, title }
}

function OKRText({ text, translations, target, instanceKey, className = '' }: { text: string; translations: TranslationMap; target?: CommentTarget; instanceKey?: string; className?: string }) {
  const english = translations[text]?.trim()
  const translated = english && english !== text.trim() ? english : ''
  const content = <><span className="block whitespace-pre-wrap">{text}</span>{translated && <><span aria-hidden>{'\n'}</span><span className="mt-0.5 block whitespace-pre-wrap font-normal text-slate-400">{translated}</span></>}</>
  return target && instanceKey
    ? <CommentableText target={target} text={`${text}${translated ? `\n${translated}` : ''}`} instanceKey={instanceKey} className={`whitespace-pre-wrap ${className}`}>{content}</CommentableText>
    : <span className={className}>{content}</span>
}

function KRDetails({ kr, translations, instanceKey }: { kr: Kr; translations: TranslationMap; instanceKey: string }) {
  const ownerTitle = krOwners(kr).map((owner) => owner.name).join('、') || '未填写负责人 / Owner not set'
  return <details data-comment-target-key={`kr:${kr.id}`} className="mt-2 rounded-lg border border-slate-100 bg-slate-50/60" open>
    <summary className="flex cursor-pointer list-none items-start gap-2 px-3 py-2 text-[11px] font-medium text-slate-700"><span className="mt-0.5 shrink-0 text-slate-300">▸</span><OKRText text={kr.title} translations={translations} target={{ type: 'kr', id: kr.id, title: kr.title }} instanceKey={`${instanceKey}:kr:${kr.id}`} className="min-w-0 flex-1" /><CommentableField target={alignmentTarget(`kr:${kr.id}:owners`, `KR 负责人：${ownerTitle}`)} instanceKey={`${instanceKey}:kr:${kr.id}:owners`} className="shrink-0 pt-0.5 font-normal text-slate-400"><PeopleInline people={krOwners(kr)} compact empty="未填写负责人 / Owner not set" /></CommentableField></summary>
    <div className="space-y-2 border-t border-slate-100 px-3 py-2">
      {kr.points.map((point) => <details key={point.id} open className="rounded-md bg-white px-2 py-1.5">
        <summary className="flex cursor-pointer items-start gap-1 text-[10px] font-medium text-slate-600"><span className="shrink-0 pt-0.5">{point.kind === 'product' ? '产品具体 KR / Product KR' : '策略具体 KR / Strategy KR'}：</span><OKRText text={point.title} translations={translations} target={{ type: 'point', id: point.id, title: point.title }} instanceKey={`${instanceKey}:point:${point.id}`} className="min-w-0 flex-1" /></summary>
        <CommentableField target={alignmentTarget(`point:${point.id}:owners`, `具体 KR 负责人：${(point.owners ?? []).map((owner) => owner.name).join('、') || '未填写'}`)} instanceKey={`${instanceKey}:point:${point.id}:owners`} className="mt-1 flex items-center gap-1 pl-3 text-[10px] text-slate-400"><span>负责人 / POC：</span><PeopleInline people={point.owners ?? []} compact empty="未填写 / Not set" /></CommentableField>
        {point.entries.map((entry) => <div key={entry.id} className="mt-1 pl-3 text-[10px] leading-5 text-slate-500"><OKRText text={entry.text} translations={translations} target={{ type: 'entry', id: entry.id, title: entry.text }} instanceKey={`${instanceKey}:entry:${entry.id}`} /></div>)}
      </details>)}
...
function DemandEditor({ initial, pendingKey, objectives, translations, people, busy, onSave, onDelete, onComment, onPendingChange }: { initial: RegionalDemand | Omit<RegionalDemand, 'id'>; pendingKey: string; objectives: Objective[]; translations: TranslationMap; people: KrOwner[]; busy: boolean; onSave: (value: RegionalDemand | Omit<RegionalDemand, 'id'>) => Promise<RegionalDemand | undefined>; onDelete?: () => void; onComment?: () => void; onPendingChange: (key: string, pending: boolean) => void }) {
  ...
  const pending = saving || (signature !== lastSavedRef.current && (!isNew || demandHasContent(value)))
  useEffect(() => { onPendingChange(pendingKey, pending) }, [onPendingChange, pending, pendingKey])
  useEffect(() => () => onPendingChange(pendingKey, false), [onPendingChange, pendingKey])
  useEffect(() => {
  ...
  const canComment = id !== 'new'
  const target = (fieldName: string, label: string, current: string) => alignmentTarget(`d:${id}:${fieldName}`, `${label}：${current || '未填写'}`)
  ...
  return <div id={commentTargetElementId({ type: 'alignment_item', id: `demand:${id}` })} data-comment-target-key={`alignment_item:demand:${id}`} className={`grid items-stretch ${DEMAND_TABLE_COLUMNS}`} role="row">
    <div className={DEMAND_TABLE_CELL} role="cell">{canComment ? <CommentableTextarea target={target('regional_okr', '区域 OKR', value.regionalOkr)} instanceKey={`demand:${id}:regional_okr`} aria-label="区域 OKR / Regional OKR" value={value.regionalOkr} onChange={(event) => field('regionalOkr', event.target.value)} className={tableInputClass} rows={5} /> : <textarea aria-label="区域 OKR / Regional OKR" value={value.regionalOkr} onChange={(event) => field('regionalOkr', event.target.value)} className={tableInputClass} rows={5} />}</div>
    <div className={DEMAND_TABLE_CELL} role="cell" onPaste={pasteRequirement} tabIndex={0} aria-busy={paste.uploading}>{canComment ? <CommentableTextarea target={target('requirement', '具体需求', value.requirement)} instanceKey={`demand:${id}:requirement`} aria-label="具体需求 / Detailed requirement" value={value.requirement} onChange={(event) => field('requirement', event.target.value)} placeholder="填写需求，或粘贴链接/图片 / Enter requirement or paste links/images" className={tableInputClass} rows={3} /> : <textarea aria-label="具体需求 / Detailed requirement" value={value.requirement} onChange={(event) => field('requirement', event.target.value)} placeholder="填写需求，或粘贴链接/图片 / Enter requirement or paste links/images" className={tableInputClass} rows={3} />}<div className="flex flex-wrap items-start gap-2 border-t border-slate-100 px-3 py-2">{canComment ? <CommentableField target={target('assets', '需求附件', [...value.docs.map((item) => item.title), ...value.images.map((item) => item.name ?? item.url)].join('、'))} instanceKey={`demand:${id}:assets`} className="flex flex-wrap items-start gap-2 pr-8"><Links value={value.docs} onChange={(next) => field('docs', next)} /><Images value={value.images} onChange={(next) => field('images', next)} maxDisplayWidth={120} pasteEnabled={false} /></CommentableField> : <><Links value={value.docs} onChange={(next) => field('docs', next)} /><Images value={value.images} onChange={(next) => field('images', next)} maxDisplayWidth={120} pasteEnabled={false} /></>}<span className={`text-[10px] ${paste.uploadError ? 'text-red-500' : 'text-slate-400'}`}>{paste.uploading ? '图片上传中… / Uploading…' : paste.uploadError || '支持粘贴链接或图片 / Paste a link or image'}</span></div></div>
    <div className={DEMAND_TABLE_CELL} role="cell">{canComment ? <CommentableField target={target('priority', '优先级', value.priority)} instanceKey={`demand:${id}:priority`}><select aria-label="优先级 / Priority" value={value.priority} onChange={(event) => field('priority', event.target.value)} className={`${tableSelectClass} pr-9`}><option value="">未选择 / Not selected</option><option value="p0">Focus item</option><option value="p1">P1</option><option value="p2">P2</option></select></CommentableField> : <select aria-label="优先级 / Priority" value={value.priority} onChange={(event) => field('priority', event.target.value)} className={tableSelectClass}><option value="">未选择 / Not selected</option><option value="p0">Focus item</option><option value="p1">P1</option><option value="p2">P2</option></select>}</div>
    <div className={`${DEMAND_TABLE_CELL} min-h-28 px-2 py-2`} role="cell">{canComment ? <CommentableField target={target('regional_poc', '区域负责人', value.regionalPocs.map((owner) => owner.name).join('、'))} instanceKey={`demand:${id}:regional_poc`} className="min-h-24 pr-8"><FeishuPeoplePickerInput owners={value.regionalPocs} options={people} preferredDepartmentKeywords={['LIVE']} onChange={(next) => field('regionalPocs', next)} /></CommentableField> : <FeishuPeoplePickerInput owners={value.regionalPocs} options={people} preferredDepartmentKeywords={['LIVE']} onChange={(next) => field('regionalPocs', next)} />}</div>
    <div className={`${DEMAND_TABLE_CELL} !bg-sky-50`} role="cell">{canComment ? <CommentableField target={target('acceptance', '是否承接', value.acceptance)} instanceKey={`demand:${id}:acceptance`}><select aria-label="是否承接 / Accepted" value={value.acceptance} onChange={(event) => field('acceptance', event.target.value)} className={`${tableSelectClass} pr-9`}><option value="yes">是 / Yes</option><option value="no">否 / No</option><option value="tbd">待定 / TBD</option></select></CommentableField> : <select aria-label="是否承接 / Accepted" value={value.acceptance} onChange={(event) => field('acceptance', event.target.value)} className={tableSelectClass}><option value="yes">是 / Yes</option><option value="no">否 / No</option><option value="tbd">待定 / TBD</option></select>}</div>
    <div className={`${DEMAND_TABLE_CELL} min-h-28 !bg-sky-50 px-2 py-2`} role="cell">{canComment ? <CommentableField target={target('platform_poc', '平台负责人', value.platformPocs.map((owner) => owner.name).join('、'))} instanceKey={`demand:${id}:platform_poc`} className="min-h-24 pr-8"><FeishuPeoplePickerInput owners={value.platformPocs} options={people} preferredDepartmentKeywords={['LIVE Platform', 'Platform', 'LIVE']} onChange={(next) => field('platformPocs', next)} /></CommentableField> : <FeishuPeoplePickerInput owners={value.platformPocs} options={people} preferredDepartmentKeywords={['LIVE Platform', 'Platform', 'LIVE']} onChange={(next) => field('platformPocs', next)} />}</div>
    <div className={`${DEMAND_TABLE_CELL} !bg-sky-50 px-2 py-2`} role="cell">{canComment ? <CommentableField target={target('plan_kr', '关联 Platform OKR', value.planKrIds.join('、'))} instanceKey={`demand:${id}:plan_kr`} className="pr-8"><PlanKRSelect value={value.planKrIds} objectives={objectives} translations={translations} onChange={(next) => field('planKrIds', next)} /></CommentableField> : <PlanKRSelect value={value.planKrIds} objectives={objectives} translations={translations} onChange={(next) => field('planKrIds', next)} />}</div>
    <div className={DEMAND_TABLE_CELL} role="cell">{canComment ? <CommentableTextarea target={target('deliverable', '交付物', value.deliverable)} instanceKey={`demand:${id}:deliverable`} aria-label="交付物 / Deliverable" value={value.deliverable} onChange={(event) => field('deliverable', event.target.value)} className={tableInputClass} rows={5} /> : <textarea aria-label="交付物 / Deliverable" value={value.deliverable} onChange={(event) => field('deliverable', event.target.value)} className={tableInputClass} rows={5} />}</div>
...
function DemandTable({ demands, drafts, objectives, translations, people, busy, onSave, onDraftSaved, onDelete, onComment, onPendingChange }: { demands: RegionalDemand[]; drafts: RegionalDemandDraft[]; objectives: Objective[]; translations: TranslationMap; people: KrOwner[]; busy: boolean; onSave: (value: RegionalDemand | Omit<RegionalDemand, 'id'>) => Promise<RegionalDemand | undefined>; onDraftSaved: (key: string) => void; onDelete: (value: RegionalDemand) => void; onComment: (value: RegionalDemand) => void; onPendingChange: (key: string, pending: boolean) => void }) {
  return <div>
    <div className="mb-3 rounded-lg border border-sky-100 bg-sky-50/70 px-3 py-2 text-xs leading-5 text-slate-600">
      <CommentableText target={alignmentTarget('section:demands:guide', '区域需求填写说明')} text="区域 OKR、具体需求、优先级、区域负责人列由区域运营填写；是否承接、平台负责人、关联 Platform OKR 列由 Platform 团队填写。Regional Operations fills in Regional OKR, Detailed Requirement, Priority, and Regional POC. The Platform team fills in Accepted, Platform POC, and Related Platform OKR." instanceKey="section:demands:guide"><><span className="block">区域 OKR、具体需求、优先级、区域负责人列由区域运营填写；是否承接、平台负责人、关联 Platform OKR 列由 Platform 团队填写。</span><span className="block text-[11px] text-slate-400">Regional Operations fills in Regional OKR, Detailed Requirement, Priority, and Regional POC. The Platform team fills in Accepted, Platform POC, and Related Platform OKR.</span></></CommentableText>
    </div>
    <div className="overflow-x-auto">
      <div className="min-w-[1360px] overflow-hidden rounded-xl border border-slate-200" role="table" aria-label="区域需求表格 / Regional requirement table">
        ...
        {demands.map((demand) => <DemandEditor key={demand.id} pendingKey={`demand:${demand.id}`} initial={demand} objectives={objectives} translations={translations} people={people} busy={busy} onSave={onSave} onDelete={() => onDelete(demand)} onComment={() => onComment(demand)} onPendingChange={onPendingChange} />)}
        {drafts.map((draft) => <DemandEditor key={draft.key} pendingKey={`draft:${draft.key}`} initial={draft.value} objectives={objectives} translations={translations} people={people} busy={busy} onSave={async (value) => { const saved = await onSave(value); if (saved) onDraftSaved(draft.key); return saved }} onPendingChange={onPendingChange} />)}
      </div>
    </div>
...
function DecisionEditor({ initial, region, people, onSave, onComment, onPendingChange }: { initial: RegionalPlanDecisionItem; region: RegionalCode; people: KrOwner[]; onSave: (value: RegionalPlanDecisionItem) => Promise<RegionalPlanDecisionItem | undefined>; onComment: () => void; onPendingChange: (key: string, pending: boolean) => void }) {
  const [value, setValue] = useState(initial)
  const [saving, setSaving] = useState(false)
  ...
  const pending = saving || signature !== lastSavedRef.current
  useEffect(() => { onPendingChange(`decision:${value.planKrId}`, pending) }, [onPendingChange, pending, value.planKrId])
  useEffect(() => () => onPendingChange(`decision:${value.planKrId}`, false), [onPendingChange, value.planKrId])
  useEffect(() => {
  ...
  const target = (fieldName: string, label: string, current: string) => alignmentTarget(`pd:${value.planKrId}:${fieldName}`, `${label}：${current || '未填写'}`)
  return <div id={commentTargetElementId({ type: 'alignment_item', id: `plan:${value.planKrId}` })} className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 shadow-[inset_4px_0_0_#f59e0b]">
    <div className="grid items-end gap-3 sm:grid-cols-2 xl:grid-cols-[8rem_minmax(12rem,1.25fr)_minmax(11rem,1fr)_minmax(14rem,1.5fr)_auto]">
      <label className={`${fieldClass} font-semibold text-amber-900`}>是否上车 / Onboard<CommentableField target={target('onboard', '是否上车', value.onboard)} instanceKey={`decision:${value.planKrId}:onboard`}><select value={value.onboard} onChange={(event) => change('onboard', event.target.value as RegionalPlanDecisionItem['onboard'])} className={`${controlClass} pr-9`}><option value="">未选择 / Not selected</option><option value="yes">是 / Yes</option><option value="no">否 / No</option></select></CommentableField></label>
      <label className={fieldClass}>上车地区 / Launch regions<CommentableField target={target('launch_regions', '上车地区', value.launchRegions.join('、'))} instanceKey={`decision:${value.planKrId}:launch_regions`}><Select mode="tags" allowClear value={value.launchRegions} onChange={(next) => change('launchRegions', next)} tokenSeparators={[',', '，', '、']} options={suggestions.map((item) => ({ label: item, value: item }))} placeholder="多选或自行输入 / Select or enter" aria-label="上车地区 / Launch regions" className="mt-1.5 w-full pr-8" style={{ minHeight: 36 }} maxTagCount="responsive" /></CommentableField></label>
      <label className={fieldClass}>区域负责人 / Regional POC<CommentableField target={target('regional_poc', '区域负责人', value.regionalPocs.map((owner) => owner.name).join('、'))} instanceKey={`decision:${value.planKrId}:regional_poc`}><div className="mt-1.5 flex h-9 items-center rounded-lg border border-slate-200 bg-white px-2 pr-9"><FeishuPeoplePickerInput small owners={value.regionalPocs} options={people} preferredDepartmentKeywords={['LIVE']} onChange={(next) => change('regionalPocs', next)} /></div></CommentableField></label>
      <label className={fieldClass}>关联区域 OKR / Related regional OKR<CommentableInput target={target('regional_okr', '关联区域 OKR', value.regionalOkr)} instanceKey={`decision:${value.planKrId}:regional_okr`} value={value.regionalOkr} onChange={(event) => change('regionalOkr', event.target.value)} className={controlClass} /></label>
...
function PlatformSection({ board, category, priority, people, busy, onDecision, onComment, onPendingChange }: { board: RegionalAlignmentBoard; category: PlatformCategory; priority: RegionalPriorityFilter; people: KrOwner[]; busy: boolean; onDecision: (value: RegionalPlanDecisionItem) => Promise<RegionalPlanDecisionItem | undefined>; onComment: (target: CommentTarget) => void; onPendingChange: (key: string, pending: boolean) => void }) {
  ...
  return <div className="space-y-3"><label className="flex justify-end text-[10px] text-slate-400"><input type="checkbox" checked={showHidden} onChange={(event) => setShowHidden(event.target.checked)} className="mr-1" />显示已移除 Platform OKR / Show removed Platform OKRs</label>
    {objectives.map((objective) => <details key={objective.id} open className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm"><summary className="flex cursor-pointer list-none items-start text-sm font-semibold text-slate-800"><OKRText text={objective.title} translations={board.translations} target={{ type: 'objective', id: objective.id, title: objective.title }} instanceKey={`part0:objective:${objective.id}`} className="min-w-0 flex-1" /></summary>{objective.krs.map((kr) => {
      ...
      return <div key={kr.id} className={`mt-3 border-t border-slate-100 pt-2 ${decision.hidden ? 'opacity-60' : ''}`}><KRDetails kr={kr} translations={board.translations} instanceKey="part0" /><DecisionEditor initial={decision} region={board.region.regionCode} people={people} onSave={onDecision} onComment={() => onComment({ type: 'alignment_item', id: `plan:${kr.id}`, title: kr.title })} onPendingChange={onPendingChange} /><button type="button" disabled={busy} onClick={() => void onDecision({ ...decision, hidden: !decision.hidden })} className="mt-1 text-[9px] text-slate-400 hover:text-red-600">{decision.hidden ? '恢复到本区域视图 / Restore to this region' : '从本区域视图移除 / Remove from this region'}</button></div>
    })}</details>)}
...
function AlignmentProjects({ board, bucket, category }: { board: RegionalAlignmentBoard; bucket: MatchBucket; category: Category }) {
  ...
  const projection = useMemo(() => projectRegionalAlignment(board), [board])
  const aligned = projection.aligned.filter(({ kr }) => categoryOf(kr) === category && (bucket === 'p0' ? priorityOf(kr) === 'p0' : priorityOf(kr) !== 'p0'))
  const pendingPlatform = projection.pendingPlatform.filter(({ kr }) => categoryOf(kr) === category)
  // Keep one KR per card. Grouping all KRs from an Objective in one card made
  // a demand linked to one KR appear related to its siblings as well.
  const objectives = (bucket === 'pending' ? pendingPlatform : aligned).map(({ objective, kr }) => ({ ...objective, krs: [kr], alignmentCardKey: `${objective.id}:${kr.id}` }))
  const demands = bucket === 'pending' ? projection.pendingDemands.map((item) => item.demand) : aligned.flatMap((item) => item.demands)
  const pendingReasons = new Map(projection.pendingDemands.map((item) => [item.demand.id, item.reasons]))
  const pendingReasonText = (demand: RegionalDemand) => (pendingReasons.get(demand.id) ?? []).map((reason) => reason === 'unmatched'
    ? '尚未匹配 Platform KR'
    : reason === 'platform_not_onboard'
      ? 'Platform 已承接，但区域未上车关联 KR'
      : reason === 'platform_not_accepted'
        ? 'Platform 明确不承接该区域需求'
        : '区域已上车关联 KR，但 Platform 尚未承接').join('；')
  if (bucket === 'pending') return <div><div className="mb-3 flex gap-1">{([{ key: 'demand', label: '区域需求 / Regional requirements' }, { key: 'platform', label: 'Platform OKR' }] as const).map((item) => <button key={item.key} type="button" onClick={() => setPendingTab(item.key)} className={`rounded-lg border px-3 py-1.5 text-[10px] ${pendingTab === item.key ? 'border-amber-300 bg-amber-50 text-amber-700' : 'border-slate-200 bg-white text-slate-500'}`}>{item.label}</button>)}</div>{pendingTab === 'demand'
    ? <section className="rounded-xl border border-slate-200 bg-white p-3">{demands.length ? demands.map((demand) => {
      const requirement = demand.requirement || demand.item || '未填写需求 / Requirement not set'
      return <div key={demand.id} className="mt-2 rounded-lg bg-amber-50 p-2 text-[10px] leading-5"><div className="font-medium text-slate-700"><CommentableText target={alignmentTarget(`d:${demand.id}:requirement`, `具体需求：${requirement}`)} text={requirement} instanceKey={`part1:pending:demand:${demand.id}:requirement`} /></div><div className="text-amber-700">{pendingReasonText(demand)}</div><CommentableField target={alignmentTarget(`d:${demand.id}:regional_poc`, `区域负责人：${demand.regionalPocs.map((owner) => owner.name).join('、') || '未填写'}`)} instanceKey={`part1:pending:demand:${demand.id}:regional_poc`} className="flex items-center gap-1 text-slate-400"><span>区域负责人 / Regional POC：</span><PeopleInline people={demand.regionalPocs} compact empty="未填写 / Not set" /></CommentableField><CommentableField target={alignmentTarget(`d:${demand.id}:platform_poc`, `平台负责人：${demand.platformPocs.map((owner) => owner.name).join('、') || '未填写'}`)} instanceKey={`part1:pending:demand:${demand.id}:platform_poc`} className="flex items-center gap-1 text-slate-400"><span>平台负责人 / Platform POC：</span><PeopleInline people={demand.platformPocs} compact empty="未填写 / Not set" /></CommentableField></div>
    }) : <p className="mt-3 text-[10px] text-slate-400">暂无待处理区域需求 / No pending regional requirements</p>}</section>
    : <section className="rounded-xl border border-slate-200 bg-white p-3">{objectives.length ? objectives.map((objective) => <details key={objective.alignmentCardKey} open className="mt-2"><summary className="flex cursor-pointer items-start text-[11px] font-semibold text-slate-700"><OKRText text={objective.title} translations={board.translations} target={{ type: 'objective', id: objective.id, title: objective.title }} instanceKey={`part1:pending:objective:${objective.id}`} className="min-w-0 flex-1" /></summary>{objective.krs.map((kr) => <KRDetails key={kr.id} kr={kr} translations={board.translations} instanceKey="part1:pending" />)}</details>) : <p className="mt-3 text-[10px] text-slate-400">暂无区域不上车的 Platform OKR / No Platform OKRs declined by the region</p>}</section>}</div>
  return <div className="space-y-3">{objectives.map((objective) => {
    ...
    return <article key={objective.alignmentCardKey} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><h3 className="text-sm font-semibold text-slate-800"><OKRText text={objective.title} translations={board.translations} target={{ type: 'objective', id: objective.id, title: objective.title }} instanceKey={`part1:${bucket}:objective:${objective.id}`} /></h3><div className="mt-3 grid gap-3 lg:grid-cols-2"><div><div className="text-[10px] font-semibold text-indigo-600">平台团队重点 / Key points for the Platform team</div>{objective.krs.map((kr) => <div key={kr.id}><KRDetails kr={kr} translations={board.translations} instanceKey={`part1:${bucket}`} /><CommentableField target={alignmentTarget(`kr:${kr.id}:owners`, `平台负责人：${krOwners(kr).map((owner) => owner.name).join('、') || '未填写'}`)} instanceKey={`part1:${bucket}:kr:${kr.id}:owners`} className="mt-1 flex items-center gap-1 text-[10px] text-slate-400"><span>平台负责人 / Platform POC：</span><PeopleInline people={krOwners(kr)} compact empty="未填写 / Not set" /></CommentableField></div>)}</div><div><div className="text-[10px] font-semibold text-emerald-600">区域团队重点 / Key points for the Regional team</div>{linkedDemands.map((demand) => {
      const requirement = demand.requirement || demand.item || '未填写需求 / Requirement not set'
      return <details key={demand.id} open className="mt-2 rounded-lg bg-emerald-50/60 p-2"><summary className="cursor-pointer text-[10px] font-medium text-slate-700"><CommentableText target={alignmentTarget(`d:${demand.id}:requirement`, `具体需求：${requirement}`)} text={requirement} instanceKey={`part1:${bucket}:demand:${demand.id}:requirement`} /></summary><CommentableField target={alignmentTarget(`d:${demand.id}:regional_poc`, `区域负责人：${demand.regionalPocs.map((owner) => owner.name).join('、') || '未填写'}`)} instanceKey={`part1:${bucket}:demand:${demand.id}:regional_poc`} className="mt-1 flex items-center gap-1 text-[10px] text-slate-400"><span>区域负责人 / Regional POC：</span><PeopleInline people={demand.regionalPocs} compact empty="未填写 / Not set" /></CommentableField></details>
    })}{linkedDemands.length === 0 && <p className="mt-2 text-[10px] text-slate-400">暂无关联区域需求 / No linked regional requirements</p>}</div></div></article>
  })}{objectives.length === 0 && demands.length === 0 && <div className="rounded-xl border border-dashed border-slate-300 px-5 py-10 text-center text-xs text-slate-400">当前条件下暂无匹配项目 / No matching projects</div>}{demands.filter((demand) => demand.planKrIds.length === 0).map((demand) => { const requirement = demand.requirement || demand.item || '未填写需求 / Requirement not set'; return <article key={demand.id} className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-3"><div className="text-[10px] font-semibold text-emerald-700">未关联 Platform KR 的区域需求 / Regional requirement without a linked Platform KR</div><div className="mt-1 text-[11px] text-slate-700"><CommentableText target={alignmentTarget(`d:${demand.id}:requirement`, `具体需求：${requirement}`)} text={requirement} instanceKey={`part1:${bucket}:unlinked:${demand.id}`} /></div></article> })}</div>
}
...
export default function RegionalAlignmentApp({ initialQuarter, initialRegion, initialCommentId = '', autoMatchAccess, onScopeChange }: { initialQuarter: string; initialRegion?: string; initialCommentId?: string; autoMatchAccess: boolean; onScopeChange?: (quarter: string, region: RegionalCode) => void }) {
  ...
  const [part0Pending, setPart0Pending] = useState<Record<string, true>>({})
  const [autoMatchStarting, setAutoMatchStarting] = useState(false)
  const autoMatchStartingRef = useRef(false)
  const [autoMatchTask, setAutoMatchTask] = useState<{ id: number; quarter: string; region: RegionalCode }>()
  ...
  useEffect(() => { setDemandDrafts([]); setPart0Pending({}); setPendingCommentSelection(undefined); setCommentTarget(undefined); setFocusedComment(undefined) }, [quarter, region])
  useEffect(() => {
    ...
    const clearOnEscape = (event: KeyboardEvent) => { if (event.key === 'Escape') setPendingCommentSelection(undefined) }
  ...
  const markPart0Pending = useCallback((key: string, pending: boolean) => {
    setPart0Pending((current) => {
  ...
  useEffect(() => {
    if (!autoMatchTask) return
    const controller = new AbortController()
    let timer: number | undefined
    const poll = async () => {
      try {
        const task = await getTask(autoMatchTask.id, controller.signal)
        if (['pending', 'executing', 'waiting'].includes(task.status)) {
          timer = window.setTimeout(() => void poll(), 2500)
          return
        }
        setAutoMatchTask(undefined)
        if (task.status !== 'done') {
          const summary = typeof task.execution_result?.summary === 'string' ? task.execution_result.summary : task.summary
          setMatchNotice('')
          setError(task.status === 'needs_human' ? `自动匹配需要人工处理${summary ? `：${summary}` : ''}` : `自动匹配失败${summary ? `：${summary}` : ''}`)
          return
        }
        const latest = await getRegionalAlignmentBoard(autoMatchTask.quarter, autoMatchTask.region)
        if (quarter === autoMatchTask.quarter && region === autoMatchTask.region) {
          setBoard(latest)
          onScopeChange?.(autoMatchTask.quarter, autoMatchTask.region)
        }
        const summary = typeof task.execution_result?.summary === 'string' ? task.execution_result.summary : task.summary
        setMatchNotice(`自动匹配已完成 / Auto match completed${summary ? ` · ${summary}` : ''}`)
      } catch (reason) {
        if (!controller.signal.aborted) {
          setAutoMatchTask(undefined)
          setMatchNotice('')
          setError(reason instanceof Error ? `自动匹配状态读取失败：${reason.message}` : '自动匹配状态读取失败 / Failed to read auto-match status')
        }
      }
    }
    void poll()
  ...
  }, [autoMatchTask, onScopeChange, quarter, region])
  ...
  const commentDocumentOrder = useMemo(() => board ? buildRegionalCommentDocumentOrder(board) : undefined, [board])
  ...
  const runAutoMatch = async () => {
    if (busy || Object.keys(part0Pending).length > 0 || autoMatchStartingRef.current || autoMatchTask) return
    autoMatchStartingRef.current = true
    setAutoMatchStarting(true)
    setError('')
    setMatchNotice('正在创建匹配任务… / Starting auto-match task…')
    try {
      const started = await startRegionalAutoMatch(quarter, region)
      setAutoMatchTask({ id: started.taskId, quarter, region })
      setMatchNotice(`Agent 正在基于最新 Part 0 重新匹配… / Agent is matching… · Task #${started.taskId}`)
    } catch (reason) {
      setMatchNotice('')
      setError(reason instanceof Error ? `自动匹配启动失败：${reason.message}` : '自动匹配启动失败 / Failed to start auto-match')
    } finally {
      autoMatchStartingRef.current = false
      setAutoMatchStarting(false)
    }
  }
  ...
  const openComments = (target?: CommentTarget) => { setPendingCommentSelection(undefined); setCommentTarget(target); setFocusedComment(undefined); setCommentReviewMode(undefined); setCommentsOpen(true) }
  ...
  return <>
    ...
    <main data-comment-target-key={board ? `page:${board.alignment.id}` : undefined} className={`mx-auto max-w-[1480px] space-y-6 px-4 py-5 sm:px-6 lg:px-8 ${commentsOpen ? 'lg:pr-[420px]' : ''}`}>
      ...
      {loading ? <div className="rounded-2xl border border-slate-200 bg-white py-24 text-center text-sm text-slate-400">正在加载区域对齐数据… / Loading…</div> : board ? <CommentInteractionProvider value={{ enabled: true, triggerMode: 'surface', selected: commentTarget, focused: focusedComment, comments, counts: commentCounts, pendingSelection: pendingCommentSelection, setPendingSelection: setPendingCommentSelection, select: openComments }}>
        <div className="space-y-6">
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
            <CollapsibleBlock title="Part 0 · Pre-alignment" level="primary" commentTarget={alignmentTarget('section:part0', 'Part 0 · Pre-alignment')} instanceKey="section:part0">
              <div className="space-y-8">
                <CollapsibleBlock title="Regional Ops Team 高优痛点&核心需求" subtitle="High-priority pain points & core requirements" level="secondary" commentTarget={alignmentTarget('section:demands', 'Regional Ops Team 高优痛点&核心需求')} instanceKey="section:demands" action={<button type="button" onClick={addDemandDraft} className="h-9 rounded-lg bg-indigo-600 px-4 text-xs font-semibold text-white hover:bg-indigo-700">新增需求 / Add requirement</button>}>
                  <DemandTable demands={board.demands} drafts={demandDrafts} objectives={objectives} translations={board.translations} people={people} busy={busy} onSave={saveDemand} onDraftSaved={(key) => setDemandDrafts((current) => current.filter((draft) => draft.key !== key))} onDelete={(demand) => void removeDemand(demand)} onComment={(demand) => openComments({ type: 'alignment_item', id: `demand:${demand.id}`, title: demand.requirement || demand.item || '区域需求 / Regional requirement' })} onPendingChange={markPart0Pending} />
                </CollapsibleBlock>
                <div className="border-t border-slate-200 pt-6">
                  <CollapsibleBlock title="Platform Team 平台团队关键项目 & 重要业务解决方案" subtitle="Key projects & important solutions" level="secondary" commentTarget={alignmentTarget('section:platform', 'Platform Team 平台团队关键项目 & 重要业务解决方案')} instanceKey="section:platform" action={<button type="button" disabled={Boolean(refreshing)} onClick={() => void refreshLiveSources('platform')} className="h-9 rounded-lg border border-indigo-200 bg-white px-4 text-xs font-semibold text-indigo-600 hover:bg-indigo-50 disabled:opacity-40">{refreshing === 'platform' ? '刷新并翻译中… / Refreshing…' : '刷新 / Refresh'}</button>}>
                    ...
                    <div className="mt-4"><PlatformSection board={board} category={platformCategory} priority={platformPriority} people={people} busy={busy} onDecision={saveDecision} onComment={openComments} onPendingChange={markPart0Pending} /></div>
                  </CollapsibleBlock>
                </div>
              </div>
            </CollapsibleBlock>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
            <CollapsibleBlock title="Part 1 · OKR Alignment" subtitle="OKR 对齐 · Regional & Platform project alignment" level="primary" commentTarget={alignmentTarget('section:part1', 'Part 1 · OKR Alignment')} instanceKey="section:part1" action={autoMatchAccess ? <button type="button" disabled={loading || busy || Object.keys(part0Pending).length > 0 || autoMatchStarting || Boolean(autoMatchTask)} onClick={() => void runAutoMatch()} title={Object.keys(part0Pending).length > 0 ? '请等待 Part 0 自动保存完成 / Wait for Part 0 to finish saving' : undefined} className="rounded-lg bg-indigo-600 px-4 py-2 text-xs font-semibold text-white disabled:opacity-40">{autoMatchStarting ? '启动中…' : autoMatchTask ? '匹配中…' : Object.keys(part0Pending).length > 0 ? '保存中…' : '自动匹配'}<br /><span className="text-[9px] font-normal opacity-80">{autoMatchStarting ? 'Starting…' : autoMatchTask ? 'Matching…' : 'Auto match'}</span></button> : undefined}>
    ...
    {board && <CommentDrawer open={commentsOpen} reviewEnabled reviewMode={commentReviewMode} quarter={quarter} alignmentId={board.alignment.id} alignmentRegion={region} sourceTab="regional-alignment" scopeLabel={`${REGIONS.find((item) => item.code === region)?.label} · ${quarter}`} objectives={[...board.plan.objectives, ...board.recap.objectives]} targetOrder={commentDocumentOrder} target={commentTarget} focusCommentId={initialCommentId} todoEnabled onStartReview={(mode) => { setCommentTarget(undefined); setCommentReviewMode(mode) }} onShowAll={() => { setCommentTarget(undefined); setFocusedComment(undefined); setCommentReviewMode(undefined) }} onClose={() => { setCommentsOpen(false); setCommentTarget(undefined); setFocusedComment(undefined); setCommentReviewMode(undefined) }} onFocusCommentChange={setFocusedComment} onCountChange={setCommentCount} onCountsChange={setCommentCounts} onCommentsChange={setComments} />}
  </>
}
