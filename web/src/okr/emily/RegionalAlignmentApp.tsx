...
function KRDetails({ kr, translations }: { kr: Kr; translations: TranslationMap }) {
  return <details className="mt-2 rounded-lg border border-slate-100 bg-slate-50/60" open>
    <summary className="flex cursor-pointer list-none items-start gap-2 px-3 py-2 text-[11px] font-medium text-slate-700"><span className="mt-0.5 shrink-0 text-slate-300">▸</span><OKRText text={kr.title} translations={translations} className="min-w-0 flex-1" /><span className="shrink-0 pt-0.5 font-normal text-slate-400">{kr.ownerName || krOwners(kr).map((owner) => owner.name).join('、') || '未填写负责人 / Owner not set'}</span></summary>
    <div className="space-y-2 border-t border-slate-100 px-3 py-2">
      {kr.points.map((point) => <details key={point.id} open className="rounded-md bg-white px-2 py-1.5">
        ...
        <div className="mt-1 pl-3 text-[10px] text-slate-400">负责人 / POC：{(point.owners ?? []).map((owner) => owner.name).join('、') || '未填写 / Not set'}</div>
...
function AlignmentProjects({ board, bucket, category }: { board: RegionalAlignmentBoard; bucket: MatchBucket; category: Category }) {
  ...
  if (bucket === 'pending') return <div><div className="mb-3 flex gap-1">{([{ key: 'demand', label: '区域需求 / Regional requirements' }, { key: 'platform', label: 'Platform OKR' }] as const).map((item) => <button key={item.key} type="button" onClick={() => setPendingTab(item.key)} className={`rounded-lg border px-3 py-1.5 text-[10px] ${pendingTab === item.key ? 'border-amber-300 bg-amber-50 text-amber-700' : 'border-slate-200 bg-white text-slate-500'}`}>{item.label}</button>)}</div>{pendingTab === 'demand'
    ? <section className="rounded-xl border border-slate-200 bg-white p-3">{demands.length ? demands.map((demand) => <div key={demand.id} className="mt-2 rounded-lg bg-slate-50 p-2 text-[10px] leading-5"><div className="font-medium text-slate-700">{demand.requirement || demand.item || '未填写需求 / Requirement not set'}</div><div className="text-slate-400">区域负责人 / Regional POC：{demand.regionalPocs.map((owner) => owner.name).join('、') || '未填写 / Not set'}</div><div className="text-slate-400">平台负责人 / Platform POC：{demand.platformPocs.map((owner) => owner.name).join('、') || '未填写 / Not set'}</div></div>) : <p className="mt-3 text-[10px] text-slate-400">暂无不承接区域需求 / No declined regional requirements</p>}</section>
  ...
  return <div className="space-y-3">{objectives.map((objective) => {
    ...
    return <article key={objective.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><h3 className="text-sm font-semibold text-slate-800"><OKRText text={objective.title} translations={board.translations} /></h3><div className="mt-3 grid gap-3 lg:grid-cols-2"><div><div className="text-[10px] font-semibold text-indigo-600">平台团队重点 / Key points for the Platform team</div>{objective.krs.map((kr) => <div key={kr.id}><KRDetails kr={kr} translations={board.translations} /><div className="mt-1 text-[10px] text-slate-400">平台负责人 / Platform POC：{kr.ownerName || krOwners(kr).map((owner) => owner.name).join('、') || '未填写 / Not set'}</div></div>)}</div><div><div className="text-[10px] font-semibold text-emerald-600">区域团队重点 / Key points for the Regional team</div>{linkedDemands.map((demand) => <details key={demand.id} open className="mt-2 rounded-lg bg-emerald-50/60 p-2"><summary className="cursor-pointer text-[10px] font-medium text-slate-700">{demand.requirement || demand.item || '未填写需求 / Requirement not set'}</summary><div className="mt-1 text-[10px] text-slate-400">区域负责人 / Regional POC：{demand.regionalPocs.map((owner) => owner.name).join('、') || '未填写 / Not set'}</div></details>)}{linkedDemands.length === 0 && <p className="mt-2 text-[10px] text-slate-400">暂无关联区域需求 / No linked regional requirements</p>}</div></div></article>
