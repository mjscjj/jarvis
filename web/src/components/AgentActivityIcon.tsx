...
const activateHint = '点击查看 Jarvis 状态与设置'
...
interface AgentActivityIconProps extends ExecutingTaskState, Omit<ButtonHTMLAttributes<HTMLButtonElement>, 'name'> {
  enabled?: boolean
  name: string
  onActivate?: () => void
...
export const AgentActivityIcon = forwardRef<HTMLButtonElement, AgentActivityIconProps>(function AgentActivityIcon({
  name,
  enabled = true,
  count,
  error,
  ...
  return (
    <button
      ...
      ref={ref}
      type="button"
      className={`agent-activity-icon${className ? ` ${className}` : ''}`}
      data-state={status}
      aria-label={enabled ? `${name}：${label}，${activateHint}` : `${name}：${activateHint}`}
      title={tooltipDisabled ? undefined : !enabled ? activateHint : `${error ? `${label}：${error}` : label} · ${activateHint}`}
      onClick={(event) => {
        onClick?.(event)
        onActivate?.()
      }}
