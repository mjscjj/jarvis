...
try {
  ...
  assert.equal(await page.getByRole('button', { name: '数据来源', exact: true }).count(), 0)
  assert.equal(await page.locator('.chat-source-chips').count(), 0)
  ...
  assert.deepEqual(firstSend.sources, [], 'legacy source selections must not affect new turns')
  await page.getByRole('button', { name: '对话入口', exact: true }).click()
