...
test('resolves removed, invalid and disabled child routes to OKR management', () => {
  ...
  assert.equal(resolveOKRTab('manage', { 'agency-okr': false }), 'manage')
  assert.equal(resolveOKRTab('weekly-fill', { 'agency-okr': true }), 'weekly-fill')
  assert.equal(resolveOKRTab('review-fill', { 'agency-okr': true }), 'review-fill')
  assert.equal(resolveOKRTab('review-meeting', { 'agency-okr': true }), 'review-meeting')
  assert.equal(resolveOKRTab('weekly-meeting', { 'agency-okr': false }), 'manage')
  assert.equal(resolveOKRTab('review-fill', { 'agency-okr': false }), 'manage')
  assert.equal(resolveOKRTab('okr-review', { 'agency-okr': true }), 'manage')
  assert.equal(resolveOKRTab('structure', { 'agency-okr': true }), 'manage')
  assert.equal(resolveOKRTab('unknown', { 'agency-okr': true }), 'manage')
})
