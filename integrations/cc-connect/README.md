...
Codex continuity is best-effort. Before the exec backend resumes a saved session, the Codex adapter verifies that its transcript is still present under the active `sessions/` store. If an account switch, manual archive, or local cleanup moved or removed that transcript, CC Connect clears only the stale agent session ID and processes the same incoming message in a fresh Codex session. It does not unarchive old history, and remote app-server sessions keep using the server's own synchronous resume/fallback path.
