...
Agent 设置和自动化都是最外层入口：Agent 设置按 M3、M5 阶段集中维护系统提示词模板、工作规则、审批规则及配置生效预览，其他 Agent 提示词也保留在该页；自动化集中管理周期与单次定时任务。设置页包含运行配置、系统任务、Skills 和共享记忆。右侧流式对话通过条件注册的 `POST /api/chat` 使用执行模型。
