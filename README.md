...
| 定时任务 | `internal/scheduledtask/` | 独立定时任务：支持指定时间执行一次、每天指定时间或每 N 分钟执行；保存冻结上下文和下次执行时间，每分钟扫描并发调用 Codex；支持 CRUD、停用、手动触发和 Agent CLI 工具 | `service.go`（计划/状态/并发/执行）、`scheduler.go`（扫描 cron）、`prompt.go`（指令与背景边界） | MySQL `scheduled_task` + 官方 Codex |
