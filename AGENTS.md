...
1. **Agent-first，工具与提示词优先。** 优先提供原子化工具，把判断、编排和审批尺度放到对应阶段的 prompt、rules 或 Skill，由 Agent 结合上下文执行；代码只保证权限、状态、幂等、调度和工具参数等机器硬边界，不把语义流程写死。
2. **MVP 优先，模块化与可编排性优先。** 从整体架构和语义所有权出发拆分模块；每个模块职责单一、接口清晰、可以独立组合，不为局部需求增加专用链路、跨层依赖或过早抽象。
3. **Markdown 优先。** 能用本地 Markdown 作为真源的文本和轻量配置，就不用数据库或重型存储；只有确需结构化查询、关联、幂等、调度、权限或状态约束时，才引入结构化存储。
...
- 代码只提供三样东西：一个可以停下来的状态（`needs_human`）、一份结构化 `question`、以及回答后恢复同一执行 Session 的入口和事件流。这些是模型判断的**载体**，必须保留；不存在独立审批对象或 approve/reject 阶段。
...
- 因为模型调整了执行方向就强制进入 `needs_human`；只有模型按审批策略判断确实需要请示时才提问。
...
- **OKR 产品数据必须提交**：`data/okr/okr.db` 以及 `data/okr/` 下新增或修改的产品资源文件都必须随本次改动提交，不得因其由运行时产生或在接手时已是脏文件而跳过。
...
- 构建或重启服务统一执行 `./scripts/jarvis-deploy --skip-pull`，它按当前系统选择 macOS launchd 或 Linux user systemd，并自带健康与就绪验证。不要自己拼 `go build` + 重启：在 macOS 上那会破坏 TCC 稳定签名，在 Linux 上会漏掉前端构建和 unit 刷新。`./scripts/rebuild-server.sh` 是 macOS 专用底层脚本（依赖 zsh、`codesign`、`launchctl`、`plutil`），在 Linux 上不可用。完整说明见 [README.md](README.md) 的「改完代码怎么生效」。  
