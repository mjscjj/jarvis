...
**修改权。** `Task` 的 `background` / 可选 `plan` 是模型语义，**可变**。Todo 来源不人为生成 `plan`；ScheduledTask/手工 Task 可以携带计划。M5 执行中发现情况变了就直接改，不必退回上游重走一遍——这些字段不是"确认时冻结的契约"，只是"当时最好的理解"。每次修改 bump `version` 并写一条 `task_event`，记下改了什么、为什么改。**留痕是记录事实，不是干涉判断，不得以"审批归模型"为由省掉。**
