...
2. 系统提示词、全阶段规则、阶段规则、工具目录、Skills、shared memory 和运行时上下文如何组装；
3. 同一语义是否已在别处存在，最终会不会重复、冲突或跨阶段泄漏；
4. 改动后的**完整有效输入**是什么，而不只是单个文件的 diff。

例如 `workrule.Service.Block()` 会把 `all.md` 无条件注入 M3、M5，并单独注入 proactive；因此写进 `all.md` 的内容必须对这些阶段全部成立。"M3 再写一条不适用"不能修复一条本来就不该进入 M3 的全局规则。

### 1.2 配置按语义所有权分层

| 内容 | 唯一真源 | 不应放置的位置 |
|---|---|---|
| 所有运行阶段无条件成立的 principal 行为偏好 | `conf/rules/all.md` | 某个阶段的补丁规则 |
| 仅某阶段成立的行为偏好 | `conf/rules/m3.md`、`conf/rules/m5.md` | `all.md` |
...
| 某来源或领域的可复用操作步骤与命令 | 对应 Skill | Go 专用分支、全阶段规则 |
...
- 一个语义只保留一个权威来源。若多个阶段都需要，抽取真正无条件成立的最小公约数；不要把某阶段的完整流程提升成全局规则。
