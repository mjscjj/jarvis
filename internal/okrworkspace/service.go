...
type PMODigest struct {
	Quarter      string             `json:"quarter"`
	Week         string             `json:"week"`
	PreviousWeek string             `json:"previous_week,omitempty"`
	Mode         string             `json:"mode"`
	SendEnabled  bool               `json:"send_enabled"`
	Summary      PMODigestSummary   `json:"summary"`
	Sections     []PMODigestSection `json:"sections"`
}

type PMODigestSummary struct {
	RiskCount      int `json:"risk_count"`
	SyncIssueCount int `json:"sync_issue_count"`
	MissingCount   int `json:"missing_count"`
	ChangedCount   int `json:"changed_count"`
}

type PMODigestSection struct {
	Kind  string          `json:"kind"`
	Title string          `json:"title"`
	Items []PMODigestItem `json:"items"`
}

type PMODigestItem struct {
...
// PMODigest composes a stable, read-only action brief from weekly page data,
// history and persisted Meego observations. It never polls Meego or writes DB.
func (s *Service) PMODigest(ctx context.Context, quarter, week string) (PMODigest, error) {
	quarter = strings.TrimSpace(quarter)
	if quarter == "" {
		return PMODigest{}, fmt.Errorf("quarter is required")
	}
	if !weekPattern.MatchString(week) {
		return PMODigest{}, fmt.Errorf("week must use YYYY-Www")
	}
	board, err := s.Board(ctx, quarter, week)
	if err != nil {
		return PMODigest{}, err
	}
	result := PMODigest{
		Quarter: quarter, Week: week, PreviousWeek: board.PreviousWeek,
		Mode: "preview_only", SendEnabled: false,
		Sections: []PMODigestSection{
			{Kind: "risk", Title: "延期或阻塞", Items: []PMODigestItem{}},
			{Kind: "sync_issue", Title: "同步异常", Items: []PMODigestItem{}},
			{Kind: "missing", Title: "本周未填写", Items: []PMODigestItem{}},
			{Kind: "changed", Title: "较上周变化", Items: []PMODigestItem{}},
		},
	}

	var snapshots []domain.MeegoSyncSnapshot
	if err := s.db.WithContext(ctx).Order("point_id").Find(&snapshots).Error; err != nil {
		return PMODigest{}, fmt.Errorf("list PMO Meego snapshots: %w", err)
	}
	snapshotByPoint := make(map[string]domain.MeegoSyncSnapshot, len(snapshots))
	for _, snapshot := range snapshots {
		snapshotByPoint[snapshot.PointID] = snapshot
	}
			...
			for _, point := range record.Points {
				...
				for _, entry := range point.Entries {
					if isRiskStatus(string(entry.Status)) {
						item := base
						item.Status, item.Detail, item.Source = string(entry.Status), entry.Text, "page"
						result.Sections[0].Items = append(result.Sections[0].Items, item)
						hasLocalRisk = true
						break
					}
				}
				...
				if strings.TrimSpace(point.MeegoWorkItemID) != "" {
					snapshot, ok := snapshotByPoint[point.ID]
					if !ok || snapshot.WorkItemID != point.MeegoWorkItemID {
						item := base
						item.Detail, item.Source = "尚无同步快照", "meego"
						result.Sections[1].Items = append(result.Sections[1].Items, item)
					} else if snapshot.LastError != "" {
						item := base
						item.Detail, item.Source = "同步失败："+snapshot.LastError, "meego"
						result.Sections[1].Items = append(result.Sections[1].Items, item)
					} else if snapshot.Week != week {
						item := base
						item.Detail, item.Source = fmt.Sprintf("快照来自 %s，当前为 %s", emptyAs(snapshot.Week, "未知周次"), week), "meego"
						result.Sections[1].Items = append(result.Sections[1].Items, item)
					}
					if ok && snapshot.Risk && !hasLocalRisk {
						item := base
						item.Status = normalizeMeegoStatus(snapshot.RemoteStatus)
						item.Detail, item.Source = firstNonEmpty(snapshot.RemoteProgress, snapshot.RemoteTitle, "Meego 标记为风险"), "meego"
						result.Sections[0].Items = append(result.Sections[0].Items, item)
					}
				}

				if !hasFilledProgress(point.Entries) {
					item := base
					item.Detail, item.Source = "本周尚未填写进展", "page"
					result.Sections[2].Items = append(result.Sections[2].Items, item)
				}

				current := progressDigest(point.Entries)
				previous := progressDigest(point.PreviousEntries)
				if current != previous && (current != "" || previous != "") {
					item := base
					item.Status, item.Source = firstProgressStatus(point.Entries), "page_history"
					switch {
					case previous == "":
						item.Detail = "本周新增：" + current
					case current == "":
						item.Detail = "本周未更新；上次：" + previous
					default:
						item.Detail = "上次：" + previous + " → 本周：" + current
					}
					result.Sections[3].Items = append(result.Sections[3].Items, item)
				}
			}
		}
	}
	for i := range result.Sections {
		sort.SliceStable(result.Sections[i].Items, func(left, right int) bool {
			a, b := result.Sections[i].Items[left], result.Sections[i].Items[right]
			return a.ObjectiveID+"\x00"+a.KRID+"\x00"+a.PointID+"\x00"+a.Source < b.ObjectiveID+"\x00"+b.KRID+"\x00"+b.PointID+"\x00"+b.Source
		})
	}
	result.Summary = PMODigestSummary{
		RiskCount: len(result.Sections[0].Items), SyncIssueCount: len(result.Sections[1].Items),
		MissingCount: len(result.Sections[2].Items), ChangedCount: len(result.Sections[3].Items),
	}
	return result, nil
}

func displayOwner(value string) string {
...
func isRiskStatus(value string) bool {
	return value == string(domain.StatusAtRisk) || value == string(domain.StatusDelayed) || value == string(domain.StatusBlocked)
}

func hasFilledProgress(entries []ProgressView) bool {
...
func progressDigest(entries []ProgressView) string {
	parts := make([]string, 0, len(entries))
	for _, entry := range entries {
		if text := strings.TrimSpace(entry.Text); text != "" {
			parts = append(parts, string(entry.Status)+" · "+text)
		}
	}
	return strings.Join(parts, "；")
}

func firstProgressStatus(entries []ProgressView) string {
	if len(entries) == 0 {
		return ""
	}
	return string(entries[0].Status)
}
