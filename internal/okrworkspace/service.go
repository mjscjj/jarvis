...
// ObjectiveManifest is the lightweight, stable inventory used by Agents that
// need to traverse a whole quarter without loading every definition at once.
// Owner counts are occurrences, not distinct people, because completion is
// audited at each KR/Point assignment site.
type ObjectiveManifest struct {
	Quarter    string                  `json:"quarter"`
	Objectives []ObjectiveManifestItem `json:"objectives"`
	Totals     ObjectiveManifestTotals `json:"totals"`
}

type ObjectiveManifestItem struct {
	ID              string `json:"id"`
	Title           string `json:"title"`
	KRCount         int    `json:"kr_count"`
	MetricCount     int    `json:"metric_count"`
	PointCount      int    `json:"point_count"`
	KROwnerCount    int    `json:"kr_owner_count"`
	PointOwnerCount int    `json:"point_owner_count"`
}

type ObjectiveManifestTotals struct {
	Objectives            int `json:"objectives"`
	KRs                   int `json:"krs"`
	Metrics               int `json:"metrics"`
	Points                int `json:"points"`
	KROwnerOccurrences    int `json:"kr_owner_occurrences"`
	PointOwnerOccurrences int `json:"point_owner_occurrences"`
}

type ObjectiveSlice struct {
	Quarter   string        `json:"quarter"`
	Objective ObjectiveView `json:"objective"`
}
...
func (s *Service) ListObjectiveManifest(ctx context.Context, quarter string) (ObjectiveManifest, error) {
	quarter = strings.TrimSpace(quarter)
	if quarter == "" {
		scope, err := s.LatestCoreScope(ctx)
		if err != nil {
			return ObjectiveManifest{}, err
		}
		quarter = scope.Quarter
	}
	if !quarterPattern.MatchString(quarter) {
		return ObjectiveManifest{}, fmt.Errorf("quarter must use YYYY-Qn")
	}
	...
	if err := s.db.WithContext(ctx).Where("quarter = ? AND plan_id = ''", quarter).Order("sort_order, id").Find(&objectives).Error; err != nil {
		return ObjectiveManifest{}, fmt.Errorf("list objective manifest: %w", err)
	}
	result := ObjectiveManifest{Quarter: quarter, Objectives: make([]ObjectiveManifestItem, 0, len(objectives))}
	result.Totals.Objectives = len(objectives)
	if len(objectives) == 0 {
		return result, nil
	}

	objectiveIDs := make([]string, 0, len(objectives))
	itemIndexByID := make(map[string]int, len(objectives))
	for _, objective := range objectives {
		objectiveIDs = append(objectiveIDs, objective.ID)
		result.Objectives = append(result.Objectives, ObjectiveManifestItem{ID: objective.ID, Title: objective.Title})
		itemIndexByID[objective.ID] = len(result.Objectives) - 1
	}

	var krs []domain.KR
	if err := s.db.WithContext(ctx).Where("objective_id IN ?", objectiveIDs).Order("objective_id, sort_order, id").Find(&krs).Error; err != nil {
		return ObjectiveManifest{}, fmt.Errorf("list manifest krs: %w", err)
	}
	result.Totals.KRs = len(krs)
	if len(krs) == 0 {
		return result, nil
	}

	krIDs := make([]string, 0, len(krs))
	objectiveByKR := make(map[string]string, len(krs))
	for _, kr := range krs {
		krIDs = append(krIDs, kr.ID)
		objectiveByKR[kr.ID] = kr.ObjectiveID
		result.Objectives[itemIndexByID[kr.ObjectiveID]].KRCount++
	}

	var metrics []domain.KRMetric
	if err := s.db.WithContext(ctx).Where("kr_id IN ?", krIDs).Find(&metrics).Error; err != nil {
		return ObjectiveManifest{}, fmt.Errorf("list manifest metrics: %w", err)
	}
	result.Totals.Metrics = len(metrics)
	for _, metric := range metrics {
		result.Objectives[itemIndexByID[objectiveByKR[metric.KRID]]].MetricCount++
	}

	var krOwners []domain.KROwner
	if err := s.db.WithContext(ctx).Where("kr_id IN ?", krIDs).Find(&krOwners).Error; err != nil {
		return ObjectiveManifest{}, fmt.Errorf("list manifest kr owners: %w", err)
	}
	result.Totals.KROwnerOccurrences = len(krOwners)
	for _, owner := range krOwners {
		result.Objectives[itemIndexByID[objectiveByKR[owner.KRID]]].KROwnerCount++
	}

	var points []domain.KRPoint
	if err := s.db.WithContext(ctx).Where("kr_id IN ?", krIDs).Find(&points).Error; err != nil {
		return ObjectiveManifest{}, fmt.Errorf("list manifest points: %w", err)
	}
	result.Totals.Points = len(points)
	if len(points) == 0 {
		return result, nil
	}
	pointIDs := make([]string, 0, len(points))
	objectiveByPoint := make(map[string]string, len(points))
	for _, point := range points {
		pointIDs = append(pointIDs, point.ID)
		objectiveID := objectiveByKR[point.KRID]
		objectiveByPoint[point.ID] = objectiveID
		result.Objectives[itemIndexByID[objectiveID]].PointCount++
	}

	var pointOwners []domain.PointOwner
	if err := s.db.WithContext(ctx).Where("point_id IN ?", pointIDs).Find(&pointOwners).Error; err != nil {
		return ObjectiveManifest{}, fmt.Errorf("list manifest point owners: %w", err)
	}
	result.Totals.PointOwnerOccurrences = len(pointOwners)
	for _, owner := range pointOwners {
		result.Objectives[itemIndexByID[objectiveByPoint[owner.PointID]]].PointOwnerCount++
	}
	return result, nil
}

func (s *Service) GetCoreObjective(ctx context.Context, id string) (ObjectiveSlice, error) {
	id = strings.TrimSpace(id)
	if id == "" {
		return ObjectiveSlice{}, fmt.Errorf("objective id is required")
	}
	var objective domain.Objective
	if err := s.db.WithContext(ctx).First(&objective, "id = ? AND plan_id = ''", id).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return ObjectiveSlice{}, ErrNotFound
		}
		return ObjectiveSlice{}, fmt.Errorf("get objective: %w", err)
	}
	if !quarterPattern.MatchString(objective.Quarter) {
		return ObjectiveSlice{}, fmt.Errorf("objective %s has invalid quarter %q", objective.ID, objective.Quarter)
	}
	...
	if err := s.db.WithContext(ctx).Where("objective_id = ?", objective.ID).Order("sort_order, id").Find(&records).Error; err != nil {
		return ObjectiveSlice{}, fmt.Errorf("list objective krs: %w", err)
	}
	...
	for _, record := range records {
		...
		if err != nil {
			return ObjectiveSlice{}, err
		}
		view.KRs = append(view.KRs, krView)
	}
	return ObjectiveSlice{Quarter: objective.Quarter, Objective: view}, nil
}

func (s *Service) GetCorePoint(ctx context.Context, id string) (CorePointView, error) {
	id = strings.TrimSpace(id)
	if id == "" {
		return CorePointView{}, fmt.Errorf("point id is required")
	}
	var point domain.KRPoint
	if err := s.db.WithContext(ctx).First(&point, "id = ?", id).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return CorePointView{}, ErrNotFound
		}
		return CorePointView{}, fmt.Errorf("get point: %w", err)
	}
	var kr domain.KR
	if err := s.db.WithContext(ctx).First(&kr, "id = ?", point.KRID).Error; err != nil {
		...
		return CorePointView{}, fmt.Errorf("get KR for point: %w", err)
	}
	var objective domain.Objective
	if err := s.db.WithContext(ctx).First(&objective, "id = ? AND plan_id = ''", kr.ObjectiveID).Error; err != nil {
		...
		return CorePointView{}, fmt.Errorf("get objective for point: %w", err)
	}
	definition, err := s.loadKRDefinition(ctx, kr, false)
	if err != nil {
		return CorePointView{}, err
	}
	for _, item := range definition.Points {
		if item.ID == id {
			return CorePointView{
				Quarter: objective.Quarter, ObjectiveID: objective.ID, ObjectiveTitle: objective.Title,
				KRID: kr.ID, KRTitle: kr.Title, Point: item,
			}, nil
		}
	}
	return CorePointView{}, ErrNotFound
