...
- 定时任务工具：jarvis-tools list-scheduled-tasks 查询，jarvis-tools create-scheduled-task --payload - 新建（stdin JSON 要带 title/instruction/context_snapshot/scheduled_at），jarvis-tools delete-scheduled-task --id N 删除。创建时把当前页面/对话相关背景放进 context_snapshot。
