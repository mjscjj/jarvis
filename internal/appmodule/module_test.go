...
func TestServiceListsAndUpdatesKnownModule(t *testing.T) {
	...
	if err != nil || len(items) != 2 || items[0].Key != "okr" || !items[0].IsEnabled || items[0].Requires == nil || items[1].Key != "weekly-report" || len(items[1].Requires) != 1 {
		t.Fatalf("List() items=%+v err=%v", items, err)
	}
	disabled := false
	updated, err := service.Update(context.Background(), "weekly-report", Input{IsEnabled: &disabled})
	if err != nil {
		t.Fatal(err)
	}
	...
	enabled, err := service.Enabled(context.Background(), "weekly-report")
	if err != nil || !enabled {
		t.Fatalf("Enabled(weekly-report) = %t, %v, want current runtime true", enabled, err)
	}
	...
	if err != nil || len(items) != 2 || items[1].IsEnabled || items[1].ConfiguredEnabled || items[1].RestartRequired {
