...
func TestCodexDeciderUsesReadOnlyStructuredContract(t *testing.T) {
	...
	decider, err := NewCodexDecider(CodexOptions{Bin: bin, Model: "fixture-model", Timeout: 2 * time.Second})
	if err != nil {
...
func TestCodexDeciderRejectsMissingSession(t *testing.T) {
	...
	decider, err := NewCodexDecider(CodexOptions{Bin: writeCodexFixture(t, resultJSON, false), Model: "fixture-model", Timeout: 2 * time.Second})
	if err != nil {
		t.Fatalf("NewCodexDecider() error = %v", err)
	}
