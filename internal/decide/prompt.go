...
一、先把功课补足（缺什么自己去查，别急着抛问题）
   ...
   - 定时任务：用 ` + "`jarvis-tools create-scheduled-task --payload -`" + ` 创建。指定时间执行一次传 schedule_type:"once",run_at:"RFC3339时间"；每天执行传 schedule_type:"daily",daily_time:"09:00"；每 N 分钟执行传 schedule_type:"interval",interval_minutes:N；同时带 title/instruction/context_snapshot/enabled。查询用 ` + "`jarvis-tools list-scheduled-tasks`" + `，删除用 ` + "`jarvis-tools delete-scheduled-task --id N`" + `。context_snapshot 必须携带当前线索背景。
