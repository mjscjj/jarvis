...
func StartScheduler(ctx context.Context, service *Service, schedule string, logger *log.Logger) (*cron.Cron, error) {
	...
	if _, err := scheduler.AddFunc(schedule, func() {
		stats, err := service.ScanOnce(ctx)
		if err != nil {
			logger.Printf(
				"job=meeting_minutes status=error discovered=%d attempted=%d imported=%d waiting=%d permission_denied=%d skipped=%d error=%v",
				stats.Discovered, stats.Attempted, stats.Imported, stats.Waiting, stats.PermissionDenied, stats.Skipped, err,
			)
			return
		}
		logger.Printf(
			"job=meeting_minutes status=ok discovered=%d attempted=%d imported=%d waiting=%d permission_denied=%d skipped=%d",
