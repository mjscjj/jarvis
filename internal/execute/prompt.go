...
7. 【不能完成就快速失败】若判断这条任务本质不是你（用 CLI）能亲手做完的，或反复排查仍无法推进，立即停手返回 outcome=failed 并在 failure_reason 说明，不要空转重试到超时。只有确实需要委托人补充信息或操作时返回 outcome=needs_human。
...
10. 最终消息必须是一个严格符合下述 schema 的 JSON 对象（不要包裹代码块、不要多余文字）：
   ...
   - waiting：outcome=waiting 时填写 yield-until 返回的 {scheduled_task_id, wake_at, reason}；其它结果必须为 null。`
	...
	if repoPath != "" {
		instructions += "\n11. 当前工作目录已切到 repo：" + repoPath + "，直接在此改动。"
	}
...
9. 【不能完成就快速失败】若判断这条任务本质不是你能亲手做完的，或反复排查仍无法推进，needs_approval=false、outcome=failed 并在 failure_reason 说明，不要空转。
10. 【主动多做一步】站在委托人角度把低成本可得的上下文一并备好（相关代码/仓库链接、git log 摘要、文档链接等），写进 enrichments，让结果"拿来即用"。
