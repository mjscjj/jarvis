...
func GetPMODigest(service *okrworkspace.Service) app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		quarter := strings.TrimSpace(c.Query("quarter"))
		week := strings.TrimSpace(c.Query("week"))
		result, err := service.PMODigest(ctx, quarter, week)
		if err != nil {
...
func GetRegionSyncDraft(service *okrworkspace.Service) app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		result, err := service.RegionSyncDraft(ctx, strings.TrimSpace(c.Query("quarter")), strings.TrimSpace(c.Query("week")), strings.TrimSpace(c.Query("region")))
		if err != nil {
...
func GetQuarterlyOKRDraft(service *okrworkspace.Service) app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		result, err := service.QuarterlyOKRDraft(ctx, strings.TrimSpace(c.Query("quarter")), strings.TrimSpace(c.Query("week")))
		if err != nil {
...
func GenerateReportDraft(service *okrworkspace.Service) app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		var input okrworkspace.GenerateReportDraftInput
		if err := decodeStrictJSON(c.Request.Body(), &input); err != nil {
			writeAPIError(c, consts.StatusBadRequest, 40050, err)
			return
		}
		result, err := service.GenerateReportDraft(ctx, input)
		if err != nil {
...
func SaveReportDraft(service *okrworkspace.Service) app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		id := strings.TrimSpace(c.Param("draft_id"))
		var input okrworkspace.SaveReportDraftInput
		if err := decodeStrictJSON(c.Request.Body(), &input); err != nil {
			writeAPIError(c, consts.StatusBadRequest, 40052, err)
			return
		}
		input.UpdatedBy = currentOKRIdentity(c).OpenID
		result, err := service.SaveReportDraft(ctx, id, input)
		if errors.Is(err, okrworkspace.ErrConflict) {
			current, currentErr := service.GenerateReportDraft(ctx, input.GenerateReportDraftInput)
			if currentErr != nil {
				writeAPIError(c, consts.StatusInternalServerError, 50052, currentErr)
				return
			}
			writeAPIConflict(c, 40952, err, current)
			return
		}
		if err != nil {
			writeAPIError(c, consts.StatusBadRequest, 40053, err)
			return
		}
		c.JSON(consts.StatusOK, map[string]any{"code": 0, "data": result})
	}
...
func GetReportDraftHistory(service *okrworkspace.Service) app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		result, err := service.ReportDraftHistory(ctx, strings.TrimSpace(c.Param("draft_id")))
		if errors.Is(err, okrworkspace.ErrNotFound) {
			writeAPIError(c, consts.StatusNotFound, 40454, err)
			return
		}
		if err != nil {
			writeAPIError(c, consts.StatusBadRequest, 40054, err)
			return
		}
		c.JSON(consts.StatusOK, map[string]any{"code": 0, "data": result})
	}
...
func RestoreReportDraft(service *okrworkspace.Service) app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		id := strings.TrimSpace(c.Param("draft_id"))
		var input okrworkspace.RestoreReportDraftInput
		if err := decodeStrictJSON(c.Request.Body(), &input); err != nil {
			writeAPIError(c, consts.StatusBadRequest, 40055, err)
			return
		}
		input.UpdatedBy = currentOKRIdentity(c).OpenID
		result, err := service.RestoreReportDraft(ctx, id, input)
		if errors.Is(err, okrworkspace.ErrConflict) {
			current, currentErr := service.GetReportDraft(ctx, id)
			if currentErr != nil {
				writeAPIError(c, consts.StatusInternalServerError, 50055, currentErr)
				return
			}
			writeAPIConflict(c, 40955, err, current)
			return
		}
		if errors.Is(err, okrworkspace.ErrNotFound) {
			writeAPIError(c, consts.StatusNotFound, 40455, err)
			return
		}
		if err != nil {
			writeAPIError(c, consts.StatusBadRequest, 40056, err)
			return
		}
		c.JSON(consts.StatusOK, map[string]any{"code": 0, "data": result})
	}
