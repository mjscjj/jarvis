...
func ConfigurePrincipal(configPath, principalOpenID, gitAuthor string) (*PrincipalConfiguration, error) {
	...
	// two keys written by older Jarvis versions before strict validation so an
	// existing ignored runtime overlay can be migrated explicitly by rerunning
	// configure-identity. Unknown fields remain errors.
