...
type LarkCLIConfig struct {
	...
	// Deprecated: accepted for old runtime YAML only; exports preserve default labels.
...
type CaptureConfig struct {
	...
	// P2PScanEnabled controls only proactive polling of direct-message history.
	// Messages already accepted by another transport keep their own route.
...
// runs_dir 下的产物路径同样要在其它工作目录里可用；而 codex 子进程的工作目录是被改的
