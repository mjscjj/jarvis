...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "server public base url absolute", mutate: func(c *Config) { c.Server.PublicBaseURL = "https://emily.example" }},
		{name: "server public base url unset", mutate: func(c *Config) { c.Server.PublicBaseURL = "" }},
		...
		{name: "capture p2p activation window", mutate: func(c *Config) { c.Capture.P2PWindowMinutes = 0 }, wantErr: "capture.p2p_activation_window_minutes"},
...
func TestValidateExtractEnabled(t *testing.T) {
	cfg := Config{
		...
		Extract: ExtractConfig{
			...
			OpenTodoLimit: 50, RecentTaskLimit: 10, MaxPromptChars: 60000,
...
func TestAuthDefaultsDisabledAndRuntimeOverrideCanEnableIt(t *testing.T) {
	configPath := writeRuntimeSettingsTestConfig(t)
	cfg, err := Load(configPath)
	if err != nil {
		t.Fatalf("Load() error = %v", err)
