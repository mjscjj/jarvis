...
func TestValidate(t *testing.T) {
	...
	valid := Config{
		...
		Capture: CaptureConfig{
			...
			MeetingScanSchedule: "@every 5m", MeetingLookbackDays: 1,
			MeetingArtifactDir: "var/meeting-minutes", MeetingMaxContentChars: 20000,
		},
	...
	tests := []struct {
		...
		{name: "meeting capture schedule", mutate: func(c *Config) { c.Capture.MeetingScanSchedule = "" }, wantErr: "meeting_scan_schedule"},
		{name: "meeting capture lookback", mutate: func(c *Config) { c.Capture.MeetingLookbackDays = 0 }, wantErr: "meeting_lookback_days"},
		{name: "meeting capture artifact dir", mutate: func(c *Config) { c.Capture.MeetingArtifactDir = "../minutes" }, wantErr: "meeting_artifact_dir"},
		{name: "meeting capture prompt limit", mutate: func(c *Config) { c.Capture.MeetingMaxContentChars = 60000 }, wantErr: "meeting_max_content_chars"},
