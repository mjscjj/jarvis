...
func TestFailuresParsesPipelineStagesAndCron(t *testing.T) {
	svc := newRuntimeDebugService(t, ""+
		...
		"pipeline 2026/07/24 15:43:01.000000 logid=log-m4 stage=m4 trigger=realtime todo_id=99 status=error error=decide failed\n"+
		"pipeline 2026/07/24 15:44:01.000000 logid=log-m5 stage=m5 trigger=queue task_id=78 version=0 status=error error=execute Task id=78: enrichments[2] content is blank\n"+
	...
	assertFailureScope(t, byLogID["log-m4"], "decide", "m4", "todo_id", "99")
	assertFailureScope(t, byLogID["log-m5"], "execute", "m5", "task_id", "78")
