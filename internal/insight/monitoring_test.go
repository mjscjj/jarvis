...
func TestMonitoringAggregatesCoarseM3AndM5Metrics(t *testing.T) {
	...
	if err := db.AutoMigrate(&domain.ExtractionRun{}, &domain.ExecutionRun{}, &domain.Task{}); err != nil {
		t.Fatal(err)
	}
	...
	if snapshot.M3.ProcessedMessages != 12 || snapshot.M3.TodosCreated != 3 || snapshot.M3.FailedRuns != 1 {
		t.Fatalf("M3 = %+v", snapshot.M3)
	}
	if snapshot.M3.AverageDurationMS == nil || *snapshot.M3.AverageDurationMS != 2000 || snapshot.M3.MaxDurationMS == nil || *snapshot.M3.MaxDurationMS != 3000 {
	...
	if snapshot.M5.ProcessedTasks != 2 || snapshot.M5.FailedRuns != 1 || snapshot.M5.AverageDurationMS == nil || *snapshot.M5.AverageDurationMS != 4000 {
