...
func TestCoreModels(t *testing.T) {
	...
	models := CoreModels()
	if got, want := len(models), 13; got != want {
	...
	want := []string{"project", "feishu_group", "person", "todo", "task", "resource", "scan_record", "principal_profile", "managed_resource", "shared_memory", "work_rule", "agent_skill", "daily_digest"}
	for i := range want {
		if got[i] != want[i] {
