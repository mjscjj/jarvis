...
func TestShutdownStartsFixedScriptOnce(t *testing.T) {
	...
	want := []string{script, "--delay-seconds", "1", "--server-pid", "42"}
	if starter.bin != "/bin/zsh" || !reflect.DeepEqual(starter.args, want) {
		t.Fatalf("Start() = %q %#v, want /bin/zsh %#v", starter.bin, starter.args, want)
	}
}
