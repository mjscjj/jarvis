...
func buildPrompt(task *domain.ScheduledTask) (string, error) {
	...
	return `你是 Jarvis 的定时任务执行代理，运行在用户本地可信环境（danger-full-access + 联网）。现在已经到达本轮执行时间，请把 TASK_INSTRUCTION 真正执行完成，而不是只给建议。
...
5. 最终用简洁中文说明实际做了什么、结果和需要人工跟进的事项。最终文本会原样保存为本任务的最近执行结果；周期任务会继续调度，一次性任务本轮后结束。
