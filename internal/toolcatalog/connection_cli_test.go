...
func TestRebuildChecksSelectedInstanceBeforeRestart(t *testing.T) {
	for _, shell := range []string{"bash", "zsh"} {
		t.Run(shell, func(t *testing.T) {
			...
			for _, rel := range []string{name, "lib/connection.sh"} {
...
case "$*" in *restart*|*kickstart*|*bootstrap*) exit 99 ;; esac
exit 0
`)
			}
			writeExecutable(t, filepath.Join(root, "bin", "go"), `#!/bin/sh
[ "$3" = show-connection ] || exit 99
...
printf '%s\n' '{"code":0,"data":{"total":1}}'
`)
			cmd := exec.Command(shell, filepath.Join(root, "scripts", name))
			cmd.Env = sanitizedEnv(os.Environ(), "JARVIS_API_BASE", "JARVIS_CONFIG_PATH", "JARVIS_DESKTOP", "JARVIS_RESOURCE_ROOT")
			cmd.Env = append(cmd.Env, "PATH="+filepath.Join(root, "bin")+":"+os.Getenv("PATH"), "FIXTURE_ROOT="+root)
			out, err := cmd.CombinedOutput()
