...
func TestOKRModuleToolsExposeGenericReads(t *testing.T) {
	...
	for _, command := range []string{"board", "replace-kr"} {
...
func TestOKRAndBizToolsExposeAtomicWrites(t *testing.T) {
	...
	tests := []struct {
		...
		{[]string{"create-follow-up", "--payload", `{"id":"follow-up-1","expected_version":0,"quarter":"2026-Q3","week":"2026-W36","topic":"事项","owners":[{"email":"owner@example.test","name":"负责人"}],"status":"not_started"}`}, http.MethodPost, "/api/biz-okr/follow-ups", ""},
		{[]string{"update-follow-up", "--id", "follow-up-1", "--payload", `{"expected_version":1,"quarter":"2026-Q3","week":"2026-W36","topic":"事项","owners":[{"email":"owner@example.test","name":"负责人"}],"status":"abandoned"}`}, http.MethodPut, "/api/biz-okr/follow-ups/follow-up-1", ""},
