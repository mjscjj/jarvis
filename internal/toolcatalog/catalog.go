...
func Block(stage string) (string, error) {
	...
	lines := []string{
		...
		"- 实体当前状态和长期事实用 `get-page` / `update-page` 读写，`list-pages` 是页面索引。`list-facts` 返回证据索引：一句锚点加原始材料指针；按 `source_kind` / `source_id` 用 `get-message`、`get-todo-event`、`get-task-event`、`get-task-run` 或 `get-captured-resource` 读取原文。",
