...
func okrReviewBlock() string {
	lines := []string{
		...
		"- okr-module-tools：通用 OKR 定义、Metric/Point/Owner 拆解和正式进展。`scope` / `board` 读季度结构；`list-objectives` / `get-objective` 支持整季度分片遍历；`projection-audit` 机械审计世界关系覆盖；`find-krs` / `get-kr` 定位 KR；`weeks`、`progress-board`、`get-weekly-kr` 读取正式周次和进展；`replace-kr` 是通用拆解写入口，当前 Preview review 阶段不调用写命令。",
