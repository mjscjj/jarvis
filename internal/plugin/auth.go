...
func (a *Authorizer) Probe(ctx context.Context, provider string) AuthStatus {
	...
	case "lark-oncall":
		bin = "bytedcli"
		args = []string{"--json", "lark-oncall", "meta", "types"}
	default:
	...
	if err == nil || hasErrorCode(raw, "AUTH_REQUIRED", "MEEGLE_AUTH_REQUIRED", "LARK_ONCALL_AUTH_REQUIRED") {
...
func (a *Authorizer) Begin(ctx context.Context, provider string) AuthStatus {
	...
	case "lark-oncall":
		args = append(args, "auth", "login", "--begin", "--session", "--feishu")
	case "meego":
		args = append(args, "meego", "login", "--begin")
	default:
		return authError(AuthUnavailable, fmt.Sprintf("unknown authorization provider %q", provider))
	}
	raw, err := a.runner.Run(ctx, "bytedcli", args...)
	if err != nil {
		return authError(AuthFailed, commandError(raw, err))
	}
	payloads := decodeJSONValues(raw)
	if len(payloads) == 0 {
		return authError(AuthFailed, "bytedcli returned invalid authorization data")
	}
	...
	if token == "" || url == "" {
		return authError(AuthFailed, "bytedcli authorization response is missing token or verification URL")
	}
...
func (a *Authorizer) Complete(ctx context.Context, provider, flowID string) AuthStatus {
	...
	case "bytedcli-session", "lark-oncall":
