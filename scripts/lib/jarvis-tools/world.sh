...
related_group, pinned, include_in_memory, is_key_group (booleans, default false).
All are optional; {} clears the binding and sets the flags false. Enabling
...
example:
  jarvis-tools update-group --id 7 --payload '{"related_group":true,"include_in_memory":true}'
EOF
      ;;
...
cmd_update_group() { cmd_payload_by_id update-group PUT /api/groups; }
