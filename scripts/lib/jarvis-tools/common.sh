...
consume_flags() {
  ...
  while [[ $# -gt 0 ]]; do
    ...
    case "$flag" in
      ...
      --source-type) variable=SOURCE_TYPE ;;
      --relation-type) variable=RELATION_TYPE ;;
      --target-type) variable=TARGET_TYPE ;;
      --target-id) variable=TARGET_ID ;;
      --node-type) variable=NODE_TYPE ;;
      --node-id) variable=NODE_ID ;;
      --node-types) variable=NODE_TYPES ;;
      --period-key) variable=PERIOD_KEY ;;
