...
启动时将旧字典一次性还原为冻结 capture；历史更早的双消息数组合并成单一 messages。仅用已保存的数据转换，不回查当前世界。运行时不保留旧格式分支。少数旧任务当年没有捕获引用消息正文，转换时保留缺口；完整档案仍可读，执行或概览需要缺失的直接证据时明确报错，不回填或编造原文。原始来源中的旧说明作为审计语义保留，不参与当前机器协议。

验证覆盖：模型输出与 schema 一致、未知字段和精确数值不丢、消息正文只存一次、直接证据默认可见而背景按需读取、原生消息 ID 查询、正文搜索、annotation 不影响 reaction、Todo→Task 完整复制、迁移可重跑，以及 UI 详情和 run 分页。
