...
declare global {
  interface Window {
    __DECK_REGISTRY__: Array<Pick<SceneDefinition, "id" | "title" | "section" | "beats" | "profile" | "theme">>;
  }
}
...
function StageNavigator({ activeIndex, goTo }: { activeIndex: number; goTo: (index: number, beat?: number) => void }) {
  return (
    <nav
      className="stage-navigator"
      aria-label="视觉预览导航"
      data-interactive
        ...
        {deckRegistry.map((scene, index) => {
          ...
          return (
            <button
              key={scene.id}
              type="button"
              aria-label={`跳转到第 ${index + 1} 页：${scene.title}`}
              aria-current={index === activeIndex ? "page" : undefined}
              className={index === activeIndex ? "active" : ""}
              tabIndex={visible ? 0 : -1}
              style={{
                opacity: visible ? Math.max(0.18, 1 - Math.abs(distance) * 0.19) : 0,
                transform: `translateY(calc(-50% + ${distance * 42}px)) scale(${index === activeIndex ? 1 : 0.72})`,
...
function StageChrome({ scene, beat }: { scene: SceneDefinition; beat: number }) {
  return (
    <>
      <header className="stage-header">
        <span className="stage-brand">JARVIS <i>/</i> WORLD MODEL</span>
        <span className="stage-section">{scene.section}</span>
      </header>
      <footer className="stage-footer">
        <span>事实不被覆盖 · 认知持续修正</span>
        ...
        <span>COMPILE THE CURRENT WORLD</span>
      </footer>
    </>
  );
}
...
function DeckStage({ scene, activeIndex, beat, print = false, goTo }: {
  ...
  return (
    <section
      className={`stage deck-stage theme-${scene.theme} ${print ? "print-stage snapshot" : ""}`}
    ...
    >
      ...
      <StageChrome scene={scene} beat={beat} />
      <main className="scene-canvas" key={scene.id}>{scene.render(beat)}</main>
      {!print && goTo && <StageNavigator activeIndex={activeIndex} goTo={goTo} />}
    </section>
  );
}
...
function RouteError({ requestedId }: { requestedId: string }) {
  return (
    <section className="stage route-error" data-route-error={requestedId}>
      ...
      <span>请使用 registry 中的稳定 scene id。</span>
    </section>
  );
}
