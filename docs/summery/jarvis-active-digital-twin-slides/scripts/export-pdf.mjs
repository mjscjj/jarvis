...
const outputPath = resolve(outputDir, "jarvis-active-digital-twin-20p.pdf");
...
try {
  ...
  await page.goto("http://127.0.0.1:4190/?scene=opening&beat=0&snapshot=1", { waitUntil: "networkidle" });
  await page.evaluate(() => document.fonts.ready);
  const scenes = await page.evaluate(() => window.__DECK_REGISTRY__);
  if (scenes.length !== 20) throw new Error(`expected 20 scenes, got ${scenes.length}`);

  const pdf = await PDFDocument.create();
  for (const scene of scenes) {
    const beat = scene.beats - 1;
    await page.goto(`http://127.0.0.1:4190/?scene=${scene.id}&beat=${beat}&snapshot=1`, { waitUntil: "networkidle" });
    await page.evaluate(() => document.fonts.ready);
