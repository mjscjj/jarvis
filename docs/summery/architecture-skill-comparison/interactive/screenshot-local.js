...
(async () => {
  ...
  // Jarvis comparison: the normal flow's verification feedback is the key
  // architecture claim, so also capture its read-back step.
  for (let i = 0; i < 6; i++) await page.keyboard.press('ArrowRight');
  await new Promise(r => setTimeout(r, 400));
  await captureNow(`${meta.modes[0]}-${meta.flows[0]}-verify-step7`);

  // Capture the first and third step of every other flow. This spot-checks
  // the pause/resume, replan, and proactive topologies in a single-mode page.
  for (const flow of meta.flows.slice(1)) {
    await page.click(`.flowtab[data-flow="${flow}"]`);
    await new Promise(r => setTimeout(r, 300));
    await captureNow(`${meta.modes[0]}-${flow}-step1`);
    await page.keyboard.press('ArrowRight');
    if (flow === 'proactive') {
      await new Promise(r => setTimeout(r, 350));
      await captureNow(`${meta.modes[0]}-${flow}-step2`);
    }
    await page.keyboard.press('ArrowRight');
    await new Promise(r => setTimeout(r, 350));
    await captureNow(`${meta.modes[0]}-${flow}-step3`);
  }
