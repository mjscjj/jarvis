...
启用 Biz OKR 身份后，`/api/okr/*` 与 `/api/biz-okr/*` 的业务写请求统一接入现有 OKR 会话校验和作者注入；登录接口和原有公开读取保留。实际来自本机、没有浏览器 Origin/Sec-Fetch-Mode 标记且未携带 OKR Cookie 的 CLI 调用继续使用 Jarvis 身份；无效或过期 Cookie 不回退为 Jarvis。评论、导出等原有显式身份校验保留，不因本机调用而豁免。未启用 Biz 身份的独立 Core 保持原有可信运行方式。
...
- Biz 组合视图读取通用 OKR 和正式 Progress，再叠加标签、评分、评论与 Meego 信息；它不是第二份 OKR 真源。评论复用同一张讨论表，但生命周期明确分为 `(quarter, week)` 周页面、`plan_id` Plan 页面和区域对齐页面作用域；各作用域不会互相污染。顶层评论可独立标记 Todo 和解决状态，Plan、区域对齐与会议页面复用同一评论字段；周会议可把未解决的 Todo 评论投影到会议摘要，但不会另建一份待办真源。评论中的 `@` 同时保存可见原文和经固定人员目录核验的完整企业邮箱。创建评论时，显式 `@` 和评论目标最近一级的已解析 Owner 共同构成收件人；同一人只通知一次，评论作者不通知。Point 已配置 Owner 时不再扩大到 KR 或 O，当前层只有未解析人员时也不向上回退；编辑评论不重发通知。通知边界直接使用已核验完整企业邮箱，通过 `feishu.app_id / cli_profile` 固定的通知机器人发送并回读 Card 2.0。通知意图与评论同事务保存，回执单独持久化；失败不回滚评论，已成功的不重发，有消息 ID 的未知结果只回读核验。原始 CLI 错误保留服务日志，页面显示简洁状态。

- “区域 OKR 对齐”按季度绑定一份 Biz OKR Plan，并只读投影上一季度 Review；五个区域分别保存需求、Platform KR 上车决定、分类顺序和 Recap 展示覆盖，不复制 O/KR/Point。Part 1 由需求的 `plan_kr_ids`、Platform 承接状态和区域上车状态实时投影，不保存第二份匹配结果。“自动匹配”会冻结最新 Part 0 和 `match_version`，创建普通 Agent Task，并通过覆盖全部需求的原子接口仅替换 `plan_kr_ids`；现场变化时整批冲突且不部分写入。分享链接仍进入同一 Biz 页面，登录用户可共同编辑和评论；`regional_auto_match_access` 同时控制按钮与自动匹配写入口。
- Biz OKR Plan 可复用当前登录人的飞书文档导出链路生成「导出OKR Plan」文档；正文按业务方向分表，仅投影 O、KR、优先级、策略具体 KR 和产品具体 KR，相同 O/KR 使用合并单元格。
