...
```text
飞书会话发现 + principal activity + related chat 轮询
...
当前没有 Bot event stream，也不存在轮询与事件流双链路合流。

## 2. 机械职责

- 全量发现会话，但首次只从当前时刻建立 checkpoint，不回溯历史。
- 扫描 `related_group=1` 会话；tier 只用于展示。
...
轮询消息使用飞书 message ID 幂等；clue 使用 `(source, external_id)` 幂等，并生成 `clue:<source>:<external_id>` 形式的 message ID。
