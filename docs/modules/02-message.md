...
- 飞书消息返回的 `message_app_link` 原样存为可空 `message.source_url`；重复采集可补齐链接，不算新增消息，也不触发抽取。
- 原样保存消息、`mentions` 元数据和外部 clue；M3 的会话输入与冻结快照继续携带
  mention 原始 JSON，供下游按需解析稳定身份。资源只登记引用元数据，不通用下载正文。
