...
审批策略只回答“下一项具体副作用是否需要先问 principal”，包括代码修改。停车状态、回答入口和副作用留痕属于执行状态机的硬约束，保留在代码中，不能由后台关闭。
...
管理后台将 M3/M5 配置放在最外层「工作设定」页面。可编辑模板保留占位符；只读生效预览展开工作规则和审批规则，并明确列出仍由运行时注入的 phase、工具、Skills、上下文和输出协议，不能把预览冒充某次真实运行的完整 Prompt。
