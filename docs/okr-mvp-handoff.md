...
- 产品：顶层导航有独立 OKR 与周报入口。OKR 负责 KR CRUD、负责人、优先级、标签和季度材料；周报负责填写、会议、评论、历史、图片、Meego、材料和催填。
...
- 编排：启动时不投影世界模型、不安装定时任务。`okr-world-projector` 只拥有稳定实体投影；`weekly-report-progress-sync`、`weekly-report-reminder` 分别拥有同步和催办语义；后端不直接调用 Meego CLI。
...
```bash
bash -n scripts/jarvis-tools scripts/okr-module-tools scripts/weekly-report-tools
go test ./...
