...
通用 OKR 插件（module: okr）
        ↑ 提供稳定的 OKR 领域能力
Agency OKR（module: agency-okr，requires: okr）
        ↑ 提供当前组织使用的打标、Plan、Review、周报和自动化体验
```

- `okr` 是可复用的通用 OKR 插件：拥有 O、KR、Metric、Point、周次、正式 Progress 和周期指标。
- `agency-okr` 是当前业务包装：拥有标签、OKR Plan、Preview/Review、评论、评分、Follow-up、催填、Meego 关联、飞书业务身份和业务 Prompt。
...
- 当前 Agency OKR 中的 OKR、每周进展、历史记录和页面能力一个都不能丢。
...
1. 让通用 OKR 在不启用 Agency 业务时也能独立保存、查询和维护完整 OKR 与正式进展。
2. 保留 Agency OKR 当前全部产品能力和历史数据，只调整语义所有权与模块门禁。
3. 让 Jarvis 在启用 OKR 后获得 OKR 原子工具与 Skills，而不是在核心代码中写 OKR 专用流程。
4. 允许 Agent 基于世界证据形成独立进展判断，并在未来把候选正式进展交给人确认后回填 OKR。
5. 让 Agency OKR 的业务 Prompt、页面和自动化可以继续演化，而不污染通用 OKR 领域。
...
通用 OKR 和 Agency OKR 的生命周期、复用范围与变化原因不同：

| 维度 | 通用 OKR 插件 | Agency OKR |
|---|---|---|
| 服务对象 | 任意使用 OKR 的组织 | 当前 Agency 业务 |
| 稳定语义 | O、KR、指标、拆解、正式进展 | 标签口径、Plan、Review、周报展示、催填、Meego |
| 数据真源 | OKR 领域表 | Agency 业务表及业务字段 |
| Agent 能力 | 通用查询、拆解、进展 CRUD | 业务打标、评审、报告、催填与业务编排 |
| 页面 | 通用 OKR 基础能力 | 当前完整 OKR Tab 与业务工作台 |
| 依赖关系 | 不依赖 Agency | 必须依赖 `okr` |
...
正式 Progress 属于通用 OKR，因为“一个 Point 在某周的正式状态、文字、附件和版本”是 OKR 领域本身的能力，并不依赖 Agency 的评审与催填方式。

### 4.2 Agency OKR 拥有的对象

- KR / Point 的 Agency 标签与打标流程
- OKR Plan
- OKR Preview AI Review
- 周报业务布局和组织口径
- 评论、评分、Follow-up
- 催填批次、缺失人员快照和提醒策略
- Meego 关联与 Agency 特有的数据聚合
- 对外飞书登录身份、业务范围与用户授权
- Report A/B/C、研发对齐等业务 Prompt 与 Agent 编排

Agency 可以读取和调用通用 OKR，但不能成为 Objective、KR 或正式 Progress 的第二真源。
...
Jarvis 世界模型可以依赖已启用 OKR 插件提供的工具能力，但核心存储和核心流水线不能依赖 OKR DTO、Agency 标签或固定业务路由。
...
1. 人继续在 Agency OKR 当前页面填写正式 Progress。
...
| on | on | 在通用 OKR 之上提供完整 Agency OKR 产品 |
...
Agency 工具应使用独立入口（目标为 `scripts/agency-okr-tools`）：

```text
tag / plan / review / comment / score / follow-up
reminder / meego / agency prompt
```
...
| `okr-agent-orchestrator` | `agency-okr` | Agency 标签、Plan、Review、报告等动态业务 Prompt |
| `weekly-report-progress-sync` | `agency-okr`（第一阶段） | 当前混合 Agency 的 Meego/周报调查，后续按证据再拆 |
| `weekly-report-reminder` | `agency-okr` | Agency 周报催填 |

模块关闭后，对应 Skill 不进入 Agent catalog/content；ScheduledTask 根据 `context_snapshot.module` 安全跳过。已有 `weekly-report` 绑定通过一次性迁移改成 `agency-okr`，不丢失任务历史。

### 8.4 Prompt 所有权

- 通用 OKR 概念和工具说明由 tool catalog 与通用 OKR Skill 维护。
- Agency 的标签口径、评审模板、报告结构和填写偏好由 Agency Skill 或可编辑业务 Prompt 维护。
...
2. 先通过窄接口明确哪些调用属于通用 OKR、哪些属于 Agency。
3. 后续把“删除 Agency 周报周期”的编排放在 Agency 层，由它依次调用通用 OKR 删除能力和 Agency 清理能力。
4. 不引入跨模块数据库事务；任一步失败立即返回，下次可幂等重跑。
5. 在拆出清理逻辑前必须用现有数据副本做回归测试，证明没有孤儿记录和额外删除。

`KRPoint.MeegoWorkItemID` / `MeegoURL` 是已知的 Agency 字段泄漏。第一阶段保留列和数据，通过 API/接口所有权隔离；等存在第二种通用外部工作项来源时，再以证据引用或开放 relation 重构，不为假想扩展提前建抽象。

## 11. 分阶段实施

### 阶段一：建立模块和接口边界

- 新增 `agency-okr` 模块并声明 `requires: [okr]`。
- 将原 `weekly-report` 模块配置、Skill 和 ScheduledTask 绑定迁移到 `agency-okr`。
- 让 `okr` 独立拥有 Week、正式 Progress 和 Weekly KR Core 的迁移与 API。
- 将 Agency API 注册归 `agency-okr`，保持当前完整页面与数据。
- 把 Progress 原子命令迁入 `okr-module-tools`，Agency 命令保留独立入口。
- 更新前端门禁、API client 和边界测试。

### 阶段二：收窄 Service 与前端组件

- 从大 `okrworkspace.Service` 提取通用 OKR 与 Agency 两组小接口。
- 将 handlers 按所有权拆文件/package，避免 Agency handler 直接调用无关通用内部实现。
- 把通用 OKR 组件与 Agency 页面壳分开；Agency 页面组合通用组件。
- 将 `internal/okrreview`、Agency auth 和业务 Prompt 明确收进 Agency 目录。
...
- 关闭或重新开启 Agency 不执行 delete、truncate 或数据复制。

### 模块边界

- `okr=on, agency-okr=off` 可以访问 O/KR/Point、Week、Progress 和周期指标。
- Agency API、Skill、ScheduledTask 和页面只在 `agency-okr=on` 时暴露。
- `okr=off, agency-okr=on` 启动失败并明确指出缺少依赖。
- 世界模型核心测试不需要构造 Agency 业务对象。

### 产品完整性

- 当前 OKR Tab 中的 OKR、标签、Plan、Review、周报、评论、评分、Follow-up、催填和 Meego 能力都仍可使用。
- 正式 Progress 的增删改查、排序、图片和文档附件行为与拆分前一致。
- 原有 ScheduledTask 历史仍保留，新运行使用 `agency-okr` 门禁。
...
> 通用 OKR 保存组织认可的目标结构和正式进展；Agency OKR 提供当前业务的操作与展示方式；Jarvis 世界模型保存现实世界的实体、关系、事实和独立进展判断。
