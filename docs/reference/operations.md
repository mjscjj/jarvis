...
该脚本构建前端与 main/chat 二进制，按当前配置解析动态服务名和端口，再通过 macOS launchd 或 Linux user systemd 只更新这一实例，最后检查主服务 readiness、首页和启用时的 Chat health。

macOS 底层 `rebuild-server.sh` 会在替换二进制前查询执行中的 Task；服务已注册但该实例 API 不可达时会拒绝重启。`--force-interrupt-running-tasks` 只允许明确中断已查到的执行任务，不能绕过 API 查询失败。Linux 重启会中断该实例的执行子进程，忙时先查任务状态。
