...
- 周报：`GET /api/weekly-report/scope|board|comments|reminder-preview|reminder-batches|meego-preview`；评论、所选周进展、Meego 观察和催办批次的写接口位于 `/api/weekly-report/*`。季度草稿、区域对齐、Report A/B/C、催填和进展巡检由 Agent 行动绑定可编辑 Prompt，再由 `okr-agent-orchestrator` 动态组合原子工具，不提供固定生成 API。
