...
- 周报：`GET /api/weekly-report/scope|board|comments|reminder-preview|reminder-batches|pmo-digest|region-sync-draft|quarterly-okr-draft|meego-preview`；评论、所选周进展、报告草稿、Meego 确认和催办批次的写接口位于 `/api/weekly-report/*`。
