...
Jarvis 增加一套独立的周期定时任务能力：用户或 M3/M4/M5 Agent 创建一条带周期、执行指令和上下文背景的任务；服务每分钟扫描已到期记录，以固定并发调用 Codex 真正执行。

本期只做 MVP：

- 支持“每天 HH:mm”和“每 N 分钟”两种周期，不开放任意 cron 表达式。
...
| `enabled` | 是否参与自动调度 |
| `status` | `active/running`，只表示当前是否占用 |
...
- 自动抢占时先推进下一次计划；无论本轮成功还是失败，结束后都回到 `active`，不会因为一次失败永久停调度。
- 手动触发不改变 `next_run_at`，禁用任务也可手动执行；运行中拒绝重复触发。
- 修改和删除运行中的任务均拒绝；修改周期会从修改时刻重新计算 `next_run_at`。
- 服务启动时把遗留 `running` 恢复为 `active`，最近一次记为失败并保留下一次计划。

## 4. Codex Prompt

周期任务和右侧对话一样使用官方 Codex、`danger-full-access` 和联网能力。Prompt 分成两个边界：
...
- 单元测试覆盖输入校验、每天/间隔下次时间计算、错过周期跳跃、提示词上下文和 scheduler 注册。
- `go test ./...`、前端 typecheck/build、`git diff --check` 通过。
- 执行真实 MySQL 迁移并重启本机服务。
- 通过真实 API 分别创建每天和每 N 分钟任务，验证修改、列表、启停、手动触发、结果回写和删除。
