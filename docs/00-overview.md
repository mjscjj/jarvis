...
- 页面分享以及飞书提问、审批和关联任务的 Notice 卡片统一使用 `server.public_base_url` 作为用户可访问入口；卡片由 `internal/uilink` 追加任务路由。未配置时使用实际监听地址（包含安装包的 `-addr` 覆盖）：macOS 使用回环地址，Linux 的通配监听实时解析默认路由对应的本机 IPv4。回环链接标注“本机访问”，需在打开链接的设备上有本地服务或端口转发。
