...
6. 证据足够时，用 `get-world-progress` 读取项目、O、KR 或 Point 与周次的现有判断，再用 `create-world-progress` 或 `update-world-progress` 持久化 Jarvis 的独立判断。
...
`WorldProgress` 不替代人工维护的正式 `KRProgress`，也不自动回填 OKR。当前可为未归档的 `project`，以及 `okr_objective`、`okr_kr`、`okr_point` 写入；写入 OKR 主体时要求通用 `okr` 模块已启用且主体确实存在，模块关闭后仍可读取历史判断。三层 OKR 判断由 Agent 根据下级进展、指标和现实证据形成，不在 Go 中写死汇总公式。不存在 OKR 专用 evidence API，也不为某个来源增加 Go 流水线。新增来源通常只需要工具和 Skill；只有必须机器强制的可靠性约束才进入代码。
